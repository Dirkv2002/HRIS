...
async function checkAndOpenApply(vacId) {
  try {
    const { data: existing, error } = await db.from('applications')
    ...
    console.error('checkAndOpenApply failed:', err);
    openApplyModal(vacId);
  }
}
...
/* ═══════════════════════════════════════════
   SIGN TAB — DOCUMENT SIGNING (by Department/Job Title)
═══════════════════════════════════════════ */
...
/* ── ADMIN: Departments -> Job titles (accordion, like competency tab) ── */
function renderSignAdminDepts() {
  const el = document.getElementById('signDeptList');
  if (!el) return;
        ...
        ${(jobs || []).map(j => {
          ...
          return `
            <div class="job-item">
              ...
              <div class="job-actions">
                <button class="job-btn" onclick="openSignJobPanel('${safeD}','${safeJ}', this)">
                  <i class="ti ti-file-upload"></i> Documents
                </button>
              </div>
            </div>
            <div class="sign-job-panel hidden" id="signJobPanel_${safeJobKey(dept,j)}"></div>`;
        }).join('')}
      </div>
    </div>`).join('');
}
...
/* ── ADMIN: Show documents for a job title + upload button ── */
async function openSignJobPanel(dept, job, btn) {
  const key = safeJobKey(dept, job);
  const panel = document.getElementById('signJobPanel_' + key);
  if (!panel) return;

  const isHidden = panel.classList.contains('hidden');

  // Hide all other open panels
  document.querySelectorAll('.sign-job-panel').forEach(p => p.classList.add('hidden'));

  if (!isHidden) return; // was open, now closed

  panel.classList.remove('hidden');
  panel.style.padding = '12px 0';
  panel.innerHTML = '<div style="font-size:12px;color:var(--text-muted)">Loading...</div>';
          ...
          <div class="flex-between">
            <div>
              ...
              <div class="sign-doc-meta ${signed ? 'sign-status-signed' : 'sign-status-pending'}">
                ...
                ${signed ? `Signed by ${sigs.length} person${sigs.length!==1?'s':''}` : 'Pending Signature'}
              </div>
            </div>
            <div style="display:flex;gap:6px">
              ...
              <button class="btn btn-danger btn-sm" onclick="deleteSignDoc('${doc.id}','${dept}','${job}',this)"><i class="ti ti-trash"></i></button>
            </div>
          </div>
        </div>`;
    }
  } else {
    docsHtml = '<p style="font-size:12px;color:var(--text-muted)">No documents uploaded for this role yet.</p>';
  }

  panel.innerHTML = `
    <div style="background:var(--surface-2);border-radius:var(--radius-md);padding:14px;margin:6px 0">
      <div class="flex-between" style="margin-bottom:10px">
        <div class="info-label">Documents for ${escapeHtml(job)}</div>
        <button class="btn btn-primary btn-sm" onclick="openAssignDocModal('${dept.replace(/'/g,"\\'")}','${job.replace(/'/g,"\\'")}')">
...
async function deleteSignDoc(docId, dept, job, btn) {
  if (!confirm('Delete this document? This will also remove all signatures recorded for it.')) return;
  ...
  openSignJobPanel(dept, job, null);
}

/* ── ASSIGN DOCUMENT MODAL (admin uploads for a dept/job) ── */
...
async function saveAssignedDocument() {
  ...
  // Refresh the panel for that job
  const key = safeJobKey(pendingAssignDept, pendingAssignJob);
  const panel = document.getElementById('signJobPanel_' + key);
  if (panel) {
    panel.classList.add('hidden');
    openSignJobPanel(pendingAssignDept, pendingAssignJob, null);
  }
}

/* ── MANAGER: My documents to sign (matched by job_title) ── */
    ...
    el.innerHTML = `
      <div class="card" style="text-align:center;padding:40px">
        ...
        <div style="font-size:12px;color:var(--text-muted);margin-top:6px">
          Your job title is not set. Ask an administrator to set your job title on your dashboard so documents can be matched to your role.
        </div>
      </div>`;
    return;
  }

  const { data: docs, error } = await db.from('sign_documents').select('*')
    .eq('job_title', myJobTitle)
    .order('created_at', { ascending: false });

  if (error) {
    el.innerHTML = `<div class="card"><p style="font-size:13px;color:var(--danger)">Error loading documents: ${escapeHtml(error.message)}</p></div>`;
    return;
  }
