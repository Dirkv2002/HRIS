/* ===========================================
   COMMUNICARE HRIS — app.js  (v11 - FULL FIX)
=========================================== */
...
function populateDashboard() {
  ...
  // FIX: Account Management only visible to admin
...
async function renderManagerApprovalsSection(vacId, apps) {
  ...
  const reviewMap = {};
  (reviews || []).forEach(r => { reviewMap[r.application_id] = { decision: r.decision, reason: r.reason || '', managerName: r.manager_name || '' }; });
  ...
  // Group reviews by candidate and show each manager's decision
  ...
  el.innerHTML = Object.entries(byVac).map(([vacId, candidates]) => {
    ...
    return `<div class="manager-job-block">
      ...
      <div class="manager-job-body">
        ${candidates.map(c => {
          // Find MY review for this candidate
...
async function openManagerCandidateModal(appId) {
  ...
  // Load MY existing review for this application
  ...
  content.innerHTML = `
    ...
    <div class="card" style="padding:16px">
      ...
      <div id="noContinueConfirm_${appId}" style="${currentDecision === 'no-continue' ? '' : 'display:none'};margin-top:10px">
        <button class="btn btn-danger" onclick="setManagerDecision('${appId}','no-continue')"><i class="ti ti-x"></i> Confirm Decision</button>
      </div>
    </div>`;
}
...
async function setManagerDecision(appId, decision) {
  ...
  // Use upsert on the composite of application_id + manager_id
  ...
  if (existing) {
    await db.from('manager_reviews').update({ decision, reason, reviewed_at: new Date().toISOString() }).eq('id', existing.id);
  } else {
    await db.from('manager_reviews').insert({
      vacancy_id: app?.vacancy_id, application_id: appId, candidate_name: app?.candidate_name,
      manager_id: currentUser.id, manager_name: fullName, decision, reason,
      department: currentProfile.department, reviewed_at: new Date().toISOString()
    });
  }
...
/* ===========================================
   ...
   FIX: Only shows slots for the candidate's
   specific applied vacancy. No invite section.
=========================================== */
async function renderCandidateBookingSlots() {
  ...
  // Find all applications at interview1 stage
  ...
  // Get the vacancy IDs from the candidate's interview1 applications
  const myVacancyIds = myApps.map(a => String(a.vacancy_id));

  // FIX: Only fetch slots that belong to the candidate's specific vacancies
  ...
  // Check if this candidate already has a booking
  ...
  if (myBooking) {
    ...
    // Find the vacancy title for context
    const myApp = myApps.find(a => String(a.vacancy_id) === String(myBooking.vacancy_id));
    myBookDiv.innerHTML = `
      <div class="my-booking-card">
        <div class="my-booking-info">
          <div class="my-booking-label"><i class="ti ti-calendar-check"></i> Your Interview Slot${myApp ? ` — ${escapeHtml(myApp.vacancy_title || '')}` : ''}</div>
    ...
    // Only show unbooked slots for candidate's vacancies
    ...
    available.forEach(slot => {
      // Find matching vacancy title for display
...
/* ===========================================
   AVAILABILITY
   FIX: upsert with onConflict so saves persist
=========================================== */
...
async function renderAvailInviteList(vacId) {
  ...
  // FIX: Load current invites properly
...
/* FIX: upsert with onConflict so saves actually persist */
async function saveAvailInvites(vacId) {
  ...
  if (error) {
    // Fallback: delete then insert if upsert fails (in case no id/unique constraint yet)
...
/* ===========================================
   ...
   FIX: slots tied to vacancy_id, reflect on
   admin side when booked by candidate
=========================================== */
...
async function openInterviewSlotsForVacancy(vacId, vacTitle) {
  ...
  // Load slots for this vacancy (reflects bookings in real time)
  const { data: slots } = await db.from('bookings')
    .select('*')
    .eq('vacancy_id', vacId)
    .order('slot_date', { ascending: true })
    .order('slot_time', { ascending: true });

  // Load shortlisted candidates (interview1 status for this vacancy)
  const { data: shortlisted } = await db.from('applications')
    .select('*')
    .eq('vacancy_id', vacId)
    .eq('status', 'interview1');
  ...
  detail.innerHTML = `
    <div class="card flex-between" style="margin-bottom:16px">
      <div>
        ...
        <div style="font-size:12px;color:var(--text-muted)">Candidates with status "1st Interview" will see these slots and can book one.</div>
      </div>
      <button class="btn btn-primary btn-sm" onclick="openAddSlotForVacancy('${vacId}')"><i class="ti ti-plus"></i> Add Slot</button>
    </div>
    ${(shortlisted || []).length > 0 ? `
    <div class="card" style="margin-bottom:16px">
      <div class="card-title"><i class="ti ti-user-check"></i> Shortlisted Candidates (can see these slots)</div>
      <div>
        ${(shortlisted || []).map(a => `
    ...
    <div class="card-title" style="margin-bottom:8px"><i class="ti ti-calendar"></i> Slots — click refresh to see latest bookings</div>
...
function renderAdminSlotCards(slots, vacId) {
  if (slots.length === 0) return `
    <div style="text-align:center;padding:30px;color:var(--text-muted)">
      ...
      No slots yet — click "Add Slot" to create interview slots for this vacancy.
    </div>`;
    ...
    return `<div class="admin-slot-card ${isBooked ? 'booked' : 'available'}">
      ...
      ${isBooked
        ? `<div class="admin-slot-candidate">
           ...
           <span class="slot-status" style="background:var(--success-light);color:var(--success);border-radius:20px;padding:3px 10px;font-size:10px;font-weight:600">Booked</span>
      ...
      <button class="btn btn-danger btn-sm" style="margin-top:6px" onclick="adminDeleteSlotForVacancy('${slot.id}','${vacId}','${escapeHtml(slot.slot_date || '')}','${escapeHtml(slot.slot_time?.slice(0,5) || '')}')"><i class="ti ti-trash"></i></button>
    </div>`;
  }).join('') + `</div>`;
}
...
async function addSlot() {
  ...
  const { error } = await db.from('bookings').insert({
    ...
    vacancy_id:  currentBookingVacancyId,
  ...
  showToast('Slot added. Shortlisted candidates can now see and book this slot.');
}

async function adminDeleteSlotForVacancy(slotId, vacId, date, time) {
  if (!confirm('Delete this slot?' + (date ? ` (${date} ${time})` : ''))) return;
