...
/* ===========================================
   EMAIL — fire and forget, never awaited
=========================================== */
...
/* FIXED: single delete + bulk insert + fire-and-forget emails */
async function informManagerOfShortlist() {
  ...
  // Delete existing shortlist entries for this vacancy then bulk-insert
  ...
  // Fire emails without awaiting
  db.from('profiles').select('email,first_name,last_name')
    .eq('role', 'manager').eq('department', vac?.department || '')
    .then(({ data: mgrs }) => {
...
/* ===========================================
   RETRIEVE DOCUMENT — BURN SIGNATURES INTO PDF
   Uses pdf-lib to draw signatures on top of PDF pages,
   producing a final merged PDF with permanent signatures.
=========================================== */
async function retrieveDocument(docId) {
  ...
  const msg = `${doc.title}\n\n${signedCount}/${totalCount} signed · ${sigs.length} placements\n\nGenerate signed PDF with signatures burned in?`;
  if (!confirm(msg)) return;

  showToast('Generating signed PDF… please wait.', 'info');

  try {
    // Check pdf-lib is available
    ...
    // We treat all signatures as being on the first page
    // (signatures are placed as % of the overlay which maps to first page)
    // If multi-page needed later, store page_number in document_signatures
    const firstPage  = pages[0];
    const { width: pageWidth, height: pageHeight } = firstPage.getSize();

    for (const sig of sigs) {
      // Convert percentage position to PDF coordinates
      // PDF origin is bottom-left; overlay origin is top-left
      const pdfX = (sig.position_x / 100) * pageWidth;
      const pdfY = pageHeight - (sig.position_y / 100) * pageHeight;
      const sigHeight = (sig.size || 40);

      if (sig.signature_type === 'draw') {
        // Drawn signature is a PNG data URL — embed as image
        try {
          const base64 = sig.signature_data.split(',')[1];
          const imgBytes = Uint8Array.from(atob(base64), c => c.charCodeAt(0));
          const pngImage = await pdfDoc.embedPng(imgBytes);
          const imgDims  = pngImage.scale(1);
          // Scale to match displayed height
          const scale    = sigHeight / imgDims.height;
          const drawW    = imgDims.width  * scale;
          const drawH    = imgDims.height * scale;
          firstPage.drawImage(pngImage, {
            x:      pdfX,
            y:      pdfY - drawH, // PDF y goes up, subtract height
            width:  drawW,
            height: drawH,
          });
        } catch(imgErr) {
          console.warn('Could not embed drawn signature image:', imgErr);
        }
      } else {
        // Typed signature — draw as text using Helvetica
        try {
          const font     = await pdfDoc.embedFont(StandardFonts.Helvetica);
          const fontSize = sigHeight * 0.5;
          firstPage.drawText(sig.signature_data || '', {
            x:        pdfX,
            y:        pdfY - fontSize,
            size:     fontSize,
            font:     font,
          ...
          console.warn('Could not embed typed signature:', txtErr);
        }
      }
    }
    ...
    link.href     = url;
    link.download = (doc.file_name || 'document').replace('.pdf', '') + '_signed.pdf';
    link.click();
    setTimeout(() => URL.revokeObjectURL(url), 5000);

    await db.from('sign_documents').update({ retrieved: true, retrieved_at: new Date().toISOString() }).eq('id', docId);
    showToast('Signed PDF downloaded with all signatures permanently embedded.');
    await renderSignAdminView();

  } catch (err) {
    console.error('PDF generation error:', err);
    // Fallback: just download original
    alert('Could not generate signed PDF: ' + err.message + '\n\nDownloading original PDF instead.');
    const link = document.createElement('a');
...
function handleDocClick(event) {
  ...
  const sig = { id: tempId, x, y, type: currentSignatureType, data: currentSignatureData, size: 40,
    signerId: currentUser.id, signerName: `${currentProfile.first_name} ${currentProfile.last_name}`.trim(), saved: false };
...
/* ==============================================
   SIGNATURE DRAG — FIXED "running away" bug

   Root cause: previously dragOffset was in screen pixels
   but the position update divided by wrap dimensions,
   causing the signature to jump away from the cursor.

   Fix: capture the signature's percentage position at
   drag start, then compute new percentage directly from
   mouse position relative to wrap, with no offset math
   that crosses coordinate spaces.
============================================== */
function sigDragStart(e) {
  ...
  // Capture cursor's position within wrap as percentages at drag start
  const startMousePctX = (e.clientX - wrapRect.left) / wrapRect.width  * 100;
  const startMousePctY = (e.clientY - wrapRect.top)  / wrapRect.height * 100;

  // Capture the signature's current percentage position
  const sig = placedSignatures.find(s => String(s.id) === String(sigId));
  if (!sig) return;
  const startSigX = sig.x;
  const startSigY = sig.y;

  const onMove = ev => {
    // Current cursor as percentage of wrap
    const curMousePctX = (ev.clientX - wrapRect.left) / wrapRect.width  * 100;
    const curMousePctY = (ev.clientY - wrapRect.top)  / wrapRect.height * 100;

    // Delta in percentage space
    const deltaPctX = curMousePctX - startMousePctX;
    const deltaPctY = curMousePctY - startMousePctY;

    const newX = Math.max(0, Math.min(97, startSigX + deltaPctX));
    const newY = Math.max(0, Math.min(97, startSigY + deltaPctY));
  ...
  const onUp = async ev => {
    ...
    sig.x = parseFloat(Math.max(0, Math.min(97, startSigX + deltaPctX)).toFixed(2));
    sig.y = parseFloat(Math.max(0, Math.min(97, startSigY + deltaPctY)).toFixed(2));
...
/* FIXED: bulk insert all at once + fire-and-forget emails */
async function confirmAssignDocument() {
  ...
  // Delete all current assignees, then bulk-insert new selection in one call
  ...
  await db.from('document_assignees').insert(rows);

  // Emails fire without blocking the UI
