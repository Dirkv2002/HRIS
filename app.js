...
async function deleteAccount(profileId, fullName) {
  ...
  // Try the SQL RPC first — this also removes auth.users
  ...
  // Delay re-render to let DB settle
...
async function sendManagerShortlistEmail(email, n, t)          { await sendEmail(email, `Shortlist ready for review — ${t}`, `Dear ${n},\n\nA shortlist of candidates for "${t}" has been submitted for your review.\n\nKind regards,\nCommunicare Human Capital Team\n${FROM_EMAIL}`); }
async function sendManagerDocumentEmail(email, n, t)           { await sendEmail(email, `Document awaiting your signature — ${t}`, `Dear ${n},\n\nA document has been assigned to you for electronic signing: "${t}". Please log in and navigate to the Sign tab.\n\nKind regards,\nCommunicare Human Capital Team\n${FROM_EMAIL}`); }
async function sendCandidateInterviewInviteEmail(email, n, t)  { await sendEmail(email, `Interview invitation — ${t}`, `Dear ${n},\n\nCongratulations! You have been shortlisted for an interview for "${t}" at Communicare. Please log in and navigate to the Bookings tab to select your preferred interview slot.\n\nKind regards,\nCommunicare Human Capital Team\n${FROM_EMAIL}`); }
...
/* FIXED: Fetches ALL signatures (not just admin's own) so manager signatures are visible */
async function renderAllDocsList() {
  ...
  for (const doc of docs) {
    ...
    // Fetch ALL signatures for this doc — not filtered by current user
        ...
        <div class="flex-between">
          <div>
            ...
            <div class="sign-doc-meta ${allSigned ? 'sign-status-signed' : 'sign-status-pending'}">
              ...
              ${signedCount}/${totalCount} assignees signed · ${sigCount} signature placement${sigCount !== 1 ? 's' : ''} saved
            </div>
            ${doc.retrieved ? `<div class="sign-doc-meta" style="color:var(--warning)"><i class="ti ti-lock"></i> Retrieved on ${new Date(doc.retrieved_at).toLocaleDateString('en-ZA')}</div>` : ''}
          </div>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
            ...
            <a href="${doc.file_url}" target="_blank" class="btn btn-secondary btn-sm"><i class="ti ti-eye"></i> View PDF</a>
            <button class="btn btn-primary btn-sm"   onclick="viewDocumentSignatures('${doc.id}','${escapeHtml(doc.title)}')"><i class="ti ti-writing"></i> View Signatures</button>
...
/* FIXED: Shows signature count per assignee */
        ...
        <div class="flex-between">
          <div>
            ...
            <div class="sign-doc-meta ${a.signed ? 'sign-status-signed' : 'sign-status-pending'}">
              ...
              ${sigCount > 0 ? ` · ${sigCount} placement${sigCount !== 1 ? 's' : ''} saved` : ''}
            </div>
...
/* View all signatures placed on a document */
async function viewDocumentSignatures(docId, docTitle) {
  const modal   = document.getElementById('candidateProfileModal');
  const content = document.getElementById('cpModalContent');
  const nameEl  = document.getElementById('cpModalName');
  nameEl.textContent = docTitle + ' — Signature Status';
  content.innerHTML  = '<div style="text-align:center;padding:20px;color:var(--text-muted)">Loading...</div>';
  modal.classList.add('open');
  ...
  if (!assignees || assignees.length === 0) {
    content.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No one has been assigned to this document yet.</p>';
    return;
  }

  content.innerHTML = `
    <div class="card-title" style="margin-bottom:12px"><i class="ti ti-users"></i> Assignees & Signatures</div>
    ${assignees.map(a => {
      ...
      return `
        <div style="padding:12px 0;border-bottom:1px solid var(--border)">
          ...
          ${userSigs.length === 0
            ? '<div style="font-size:12px;color:var(--text-muted);font-style:italic">No signature placements yet.</div>'
            : `<div style="font-size:12px;color:var(--text-muted);margin-bottom:6px">${userSigs.length} placement${userSigs.length !== 1 ? 's' : ''} at: ${userSigs.map(s => `(${s.position_x.toFixed(1)}%, ${s.position_y.toFixed(1)}%)`).join(', ')}</div>
               <div style="display:flex;gap:6px;flex-wrap:wrap">
                 ${userSigs.map(s => s.signature_type === 'draw'
                   ...
                   : `<span style="font-family:'Dancing Script',cursive;font-size:18px;color:#003366;border:1px solid var(--border);border-radius:4px;padding:2px 8px;background:white">${escapeHtml(s.signature_data)}</span>`
                 ).join('')}
               </div>`
          }
        </div>`;
...
async function retrieveDocument(docId) {
  ...
  const message = `Document: ${doc.title}\n\nAssignees: ${signedCount}/${totalCount} have signed\nSignature placements saved: ${sigCount}\n\nNote: The downloaded PDF is the original. All signature positions are saved in the database for audit purposes. Use "View Signatures" to review them.\n\nDownload the PDF now?`;
  ...
  showToast('Document downloaded. Signature data preserved in database.');
  await renderSignAdminView();
}
...
async function openSigningModal(docId, assigneeId, alreadySigned, isRetrieved) {
  ...
  // Load only THIS user's own signatures for placement display
  ...
  placedSignatures = (sigData || []).map(s => ({
    ...
    signerId: currentUser.id, signerName: `${currentProfile.first_name} ${currentProfile.last_name}`.trim(), saved: true
  }));
...
function initSignatureCanvas() {
  ...
  signatureCtx.strokeStyle = '#003366'; signatureCtx.lineWidth = 2.5; signatureCtx.lineCap = 'round'; signatureCtx.lineJoin = 'round';
  const getPos = e => {
...
function renderPlacedSignatures() {
  ...
  placedSignatures.forEach(sig => {
    const div = document.createElement('div'); div.className = 'placed-sig-el'; div.id = 'sigEl_' + sig.id;
    div.style.left = sig.x + '%'; div.style.top = sig.y + '%'; div.style.pointerEvents = canEdit ? 'auto' : 'none';
    const fontSize = (sig.size || 40) * 0.6; const isOwn = sig.signerId === currentUser.id;
...
function sigDragStart(e) {
  ...
  const onMove = ev => {
    ...
    el.style.left = Math.max(0, Math.min(100, newX)) + '%'; el.style.top = Math.max(0, Math.min(100, newY)) + '%';
  };
  const onUp = async ev => {
