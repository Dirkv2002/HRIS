/* ═══════════════════════════════════════════
   COMMUNICARE HRIS — app.js (Part 1 of 2)
...
// New state for shortlisting flow
let shortlistDraft     = {}; // appId -> 'pending'|'shortlist'|'longlist'|'rejected'
let pendingReplaceVacId = null;
let selectedSignManagerId = null;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (roleSelect) {
    roleSelect.addEventListener('change', () => {
      ...
      if (deptGroup) {
        deptGroup.classList.toggle('hidden', roleSelect.value !== 'manager');
      }
    });
  }
...
function populateDashboard() {
  ...
  const sidebarAv = document.getElementById('sidebarAvatarText');
  ...
  if (currentProfile.avatar_url) {
    setAvatarImage(currentProfile.avatar_url);
  } else {
    avatarCircle.innerHTML = initials;
    sidebarAv.innerHTML    = initials;
  }
  ...
  // Render right-hand dashboard card based on role
  renderDashboardRightCard();
}

/* ── DASHBOARD RIGHT CARD: Application History (candidate) or Job Info (admin/manager) ── */
async function renderDashboardRightCard() {
  ...
  if (userRole === 'candidate') {
        ...
        <div class="app-history-item">
          <div>
            ...
            <div class="app-history-meta">
              Applied ${new Date(app.submitted_at).toLocaleDateString('en-ZA', { day:'numeric', month:'short', year:'numeric' })}
            </div>
          </div>
    ...
    // Admin / Manager: Job Information card
    card.innerHTML = `
      <div class="card-title accent-blue"><i class="ti ti-briefcase"></i> Job Information</div>
      <div class="info-row"><span class="key">Job Title</span><span class="val" id="infoJobTitle">—</span></div>
      <div class="info-row"><span class="key">Department</span><span class="val" id="infoJobDept">—</span></div>
      <div class="info-row"><span class="key">Reference No.</span><span class="val" id="infoJobRef">—</span></div>
      <div class="info-row"><span class="key">Application Date</span><span class="val" id="infoJobDate">—</span></div>
      <div style="margin-top:10px">
        <div class="info-label" style="margin-bottom:6px">Job Description</div>
        <div style="font-size:13px;color:var(--text-secondary);line-height:1.7" id="infoJobDesc">—</div>
      </div>
      ${userRole === 'admin' ? `
        <div style="margin-top:14px">
          <button class="btn btn-secondary" onclick="toggleEditJob()">
            <i class="ti ti-edit"></i> Edit Job Info
          </button>
          <div id="editJobForm" class="hidden" style="margin-top:16px">
            <div class="grid-2">
              <div class="form-group"><label>Job Title</label><input type="text" id="editJobTitle"/></div>
              <div class="form-group">
                ...
                <select id="editJobDept">
                  ...
                  <option>Property Development and Investments</option>
                  <option>Facilities Management</option>
                </select>
              </div>
              <div class="form-group"><label>Reference No.</label><input type="text" id="editJobRef"/></div>
            </div>
            <div class="form-group"><label>Job Description</label><textarea id="editJobDesc" rows="3"></textarea></div>
...
async function uploadAvatar(e) {
  ...
  if (error) {
    ...
    showToast('Photo preview shown locally — storage upload failed: ' + error.message, 'error');
    return;
  }
  ...
  if (data?.publicUrl) {
    ...
    if (updateErr) {
      showToast('Could not save photo: ' + updateErr.message, 'error');
      return;
    }
...
function setAvatarImage(src) {
  ...
  const sb = document.getElementById('sidebarAvatarText');
  if (c)  c.innerHTML  = `<img src="${src}" alt="Profile"/>`;
  if (sb) sb.innerHTML = `<img src="${src}" alt="Profile"/>`;
}
...
const stageMessages = {
  reviewing:  'Congratulations, your application has progressed to the Reviewing stage. Your CV has passed the baseline criteria and the HC team is currently reviewing your profile.',
  interview1: 'Congratulations, you have been chosen for a 1st Interview! Please head to the Bookings tab to schedule your interview slot.',
  assessment: 'Congratulations, you have been selected for the Assessment stage. Please head to the Assessments tab to complete your assessment.',
  interview2: 'Congratulations, you have progressed to the 2nd Interview stage! The HC team will be in touch regarding your in-person interview.',
  final:      'Congratulations, you have progressed to the Final Assessment stage. You are among the top candidates for this role.',
  offered:    'Congratulations, your application has reached the Offer stage! The HC team will contact you with further details.',
...
function showCongratsPopup(newStatus) {
  ...
  const title = newStatus === 'rejected' ? 'Application Update' : 'Congratulations!';
  document.getElementById('congratsTitle').textContent = title;
  document.getElementById('congratsText').textContent  = msg;

  const emoji = document.querySelector('#congratsModal > div > div');
  if (emoji) emoji.textContent = newStatus === 'rejected' ? '📩' : '🎉';
          ...
          return `
            <div class="applicant-row">
              ...
              <div class="applicant-row-actions">
                ...
                <button class="btn btn-secondary btn-sm" onclick="openCandidateProfile('${app.id}')">
      ...
      <div class="progress-vacancy-header" onclick="toggleProgressVacancy(this)">
        <div>
          ...
          <div class="progress-vacancy-meta">
            ${escapeHtml(vac.department || '')} · ${vacApps.length} applicant${vacApps.length !== 1 ? 's' : ''}
          </div>
        </div>
...
/* ── CANDIDATE PROFILE MODAL (Admin: full profile + docs + screening) ── */
    ...
    <div class="card" style="margin-bottom:16px;padding:16px">
      ...
      ${profile.cv_url || app.cv_url
        ? `<div style="margin-top:12px">
            <a href="${profile.cv_url || app.cv_url}" target="_blank" class="btn btn-secondary btn-sm">
              <i class="ti ti-file-cv"></i> View CV
            </a>
          </div>` : '<div style="margin-top:12px;font-size:12px;color:var(--text-muted)">No CV uploaded.</div>'}
    </div>` : ''}
...
/* ── STATUS CHANGE WITH CONFIRM (used for direct status edits if needed) ── */
const nextStatusMap = {
  applied:    'reviewing',
          ...
          <div class="strategy-item">
            <div class="strategy-item-header">
              ...
              ${(userRole === 'admin' || s.created_by === currentUser.id)
                ? `<button class="btn btn-danger btn-sm" onclick="deleteStrategy('${s.id}')"><i class="ti ti-trash"></i></button>` : ''}
            </div>
            ...
            <div style="margin-bottom:10px">
              ...
              <div style="font-size:13px;color:var(--text-secondary);line-height:1.6">${escapeHtml(s.job_description || '—')}</div>
            </div>
            <div class="grid-2">
              <div>
                <div class="report-label" style="margin-bottom:6px">Green Flags</div>
                ${(s.green_flags || '').split('\n').filter(Boolean).map(g => `<span class="tag green">✓ ${escapeHtml(g.trim())}</span>`).join('')}
              </div>
              <div>
                <div class="report-label" style="margin-bottom:6px">Red Flags</div>
                ${(s.red_flags || '').split('\n').filter(Boolean).map(r => `<span class="tag red">✕ ${escapeHtml(r.trim())}</span>`).join('')}
              </div>
            </div>
          </div>`).join('')}
      </div>
...
function renderDepts() {
  ...
  for (const [dept, jobs] of Object.entries(depts)) {
    ...
    const jobRows = (jobs || []).map(j => {
      ...
      return `
        <div class="job-item">
          <span class="job-name"><i class="ti ti-point-filled"></i>${escapeHtml(j)} ${pdfBadges}</span>
          ${adminBtns}
        </div>`;
    }).join('');
  ...
  grid.innerHTML = posted.map(v => `
    <div class="vacancy-card">
      ...
      <div class="vacancy-footer">
        <span></span>
        <button class="btn btn-primary" onclick="checkAndOpenApply('${v.id}')">
  ...
  el.innerHTML = questions.map((q, qi) => `
    <div class="sq-item">
      ...
      <div class="form-group" style="margin-bottom:10px">
        <input type="text" value="${escapeHtml(q.q||'')}" placeholder="Enter question..." onchange="updateSQQuestion(${qi},this.value)"/>
      </div>
      <div class="sq-options-builder">
        ${(q.opts||['','']).map((opt,oi) => `
          <div class="sq-option-row">
            <input type="radio" name="sqCorrect_${qi}" value="${oi}" class="sq-correct-radio"
              ${q.correct===oi?'checked':''} onchange="updateSQCorrect(${qi},${oi})" title="Mark as correct"/>
...
function removeSQQuestion(qi) {
  const vac = vacanciesData.find(v => String(v.id) === String(currentVacancyId));
  if (vac?.screening_questions) { vac.screening_questions.splice(qi,1); renderSQList(vac.screening_questions); }
}
...
/* ═══════════════════════════════════════════
   APPLY MODAL — WITH ONE-APPLICATION-PER-VACANCY CHECK
═══════════════════════════════════════════ */
async function checkAndOpenApply(vacId) {
  // Check if candidate has already applied to ANY vacancy with the same id
  const { data: existing } = await db.from('applications')
    .select('id, vacancy_id')
    .eq('user_id', currentUser.id)
    .eq('vacancy_id', String(vacId));

  if (existing && existing.length > 0) {
    pendingReplaceVacId = vacId;
    document.getElementById('replaceAppYes').onclick = () => {
      closeModal('replaceAppModal');
...
async function submitApplication() {
  ...
  if (applyCVFile) {
    ...
    // Fall back to profile CV if they didn't upload a new one
    cvUrl = currentProfile.cv_url || '';
  }

  // Check for existing application to this vacancy — replace it
  const { data: existing } = await db.from('applications')
    .select('id').eq('user_id', currentUser.id).eq('vacancy_id', String(currentVacancyId));

  if (existing && existing.length > 0) {
    await db.from('applications').delete().eq('id', existing[0].id);
    await db.from('shortlists').delete().eq('application_id', existing[0].id);
  }

  const { error } = await db.from('applications').insert({
    vacancy_id:      String(currentVacancyId),
...
/* ═══════════════════════════════════════════
   APPLICANTS MODAL — 4-COLUMN SHORTLIST WORKFLOW
═══════════════════════════════════════════ */
async function openApplicantListModal(vacId) {
  ...
  // Check for existing shortlist entries (in case this was opened before)
  ...
  // Initialise draft: use existing shortlist data if present, else 'pending'
  shortlistDraft = {};
  apps.forEach(a => {
    shortlistDraft[a.id] = shortlistMap[a.id] || 'pending';
  });
...
function renderApplicantColumns(apps) {
  ...
  const renderChip = (a, currentList) => {
    const moveOptions = ['pending','shortlist','longlist','rejected']
      .filter(l => l !== currentList)
      .map(l => {
    ...
    return `
      <div class="applicant-chip">
        <div class="applicant-chip-top">
          <span class="applicant-chip-name" style="cursor:pointer" onclick="openCandidateProfile('${a.id}')">
            ${escapeHtml(a.candidate_name || 'Candidate')}
          </span>
  ...
  const renderCol = (arr, listType) => arr.length === 0
    ? '<div style="font-size:12px;color:var(--text-muted);padding:8px 0">None</div>'
  ...
  btnWrap.style.display = (pending.length === 0 && apps.length > 0) ? 'block' : 'none';

  // Store apps for re-render on move
...
async function finaliseApplicationStatuses() {
  ...
  if (!confirm('Update application statuses for all candidates? This will send emails where applicable and notify managers of shortlisted candidates.')) return;
  ...
  for (const app of apps) {
    ...
    if (listType === 'rejected') {
      ...
      // Create or update shortlist row
      const { data: existing } = await db.from('shortlists').select('id').eq('application_id', app.id).maybeSingle();
      if (existing) {
        await db.from('shortlists').update({
          list_type: 'shortlist', updated_at: new Date().toISOString()
        }).eq('id', existing.id);
      } else {
        await db.from('shortlists').insert({
          vacancy_id: String(currentVacancyId),
          vacancy_title: vac?.title || app.vacancy_title,
    ...
    // For longlist/rejected, remove from shortlists table if present (in case moved back)
    if (listType !== 'shortlist') {
      await db.from('shortlists').delete().eq('application_id', app.id);
    }
  }

  showToast('Application statuses updated successfully.');
  closeModal('applicantListModal');
  await renderProgressAdmin();
}

/* ═══════════════════════════════════════════
   MANAGER SHORTLIST VIEW (Apply tab for managers)
═══════════════════════════════════════════ */
async function renderManagerShortlists() {
  ...
  if (!myDept) {
    el.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No department assigned to your profile.</p>';
    return;
  }

  // Get shortlists for vacancies in this manager's department
  const { data: vacs } = await db.from('vacancies').select('*').eq('department', myDept);
  const vacIds = (vacs || []).map(v => String(v.id));

  if (vacIds.length === 0) {
    el.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No vacancies found for your department.</p>';
    return;
  }
  ...
  (shortlists || []).forEach(s => {
    if (!byVac[s.vacancy_id]) byVac[s.vacancy_id] = [];
    byVac[s.vacancy_id].push(s);
  });

  if (Object.keys(byVac).length === 0) {
    el.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No shortlisted candidates for your department yet.</p>';
    return;
  }
      ...
      <div class="manager-job-block">
        ...
        <div class="manager-job-body">
          ${candidates.map(c => {
            ...
            const icon = decision === 'continue'
              ? '<i class="ti ti-check manager-decision-icon continue"></i>'
              : decision === 'no-continue'
              ? '<i class="ti ti-x manager-decision-icon no-continue"></i>'
              : '';
            return `
              <div class="manager-candidate-row" onclick="openManagerCandidateModal('${c.application_id}','${c.id}')">
...
function toggleManagerJob(header) {
  const body = header.nextElementSibling;
  const icon = header.querySelector('.chevron');
  body.classList.toggle('open');
  if (icon) icon.classList.toggle('open');
...
/* ── MANAGER CANDIDATE MODAL ── */
async function openManagerCandidateModal(appId, shortlistId) {
  ...
  content.innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px;padding:16px">
      ...
      <div class="info-row"><span class="key">Nationality</span><span class="val">${escapeHtml(profile.nationality || '—')}</span></div>
      ${profile.cv_url || app.cv_url
        ? `<div style="margin-top:12px"><a href="${profile.cv_url || app.cv_url}" target="_blank" class="btn btn-secondary btn-sm"><i class="ti ti-file-cv"></i> View CV</a></div>`
        : '<div style="margin-top:12px;font-size:12px;color:var(--text-muted)">No CV uploaded.</div>'}
    </div>` : ''}
...
async function setManagerDecision(appId, decision) {
  ...
  if (existing) {
    await db.from('manager_reviews').update({
      decision, reviewed_at: new Date().toISOString()
    }).eq('id', existing.id);
  } else {
    await db.from('manager_reviews').insert({
  ...
  showToast(decision === 'continue' ? 'Marked: Continue with candidate.' : 'Marked: Do not continue.');
...
async function renderSlots() {
  ...
  if (slotsData.length === 0) {
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:30px;color:var(--text-muted)">
      No interview slots available yet.${userRole === 'admin' ? ' Click "Add Slot" to create one.' : ''}
    </div>`;
    return;
  }
  ...
  slotsData.forEach(slot => {
    ...
    if (isMyBooking) {
      ...
      actionHtml = `<div style="margin-top:10px">
        <button class="btn btn-danger btn-full btn-sm" onclick="confirmCancelInterview('${slot.id}','${slot.slot_date}','${slot.slot_time.slice(0,5)}')">
...
function confirmCancelInterview(slotId, date, time) {
  ...
  document.getElementById('cancelInterviewText').innerHTML =
    `Are you sure you want to cancel the interview for <strong>${escapeHtml(fullName)}</strong> on <strong>${date} at ${time}</strong>?`;
...
async function addSlot() {
  ...
  if (error) {
    alert('Could not add slot: ' + error.message);
    return;
  }
...
async function renderAvailabilityCalendar() {
  ...
  myAvailability = new Set(
    allAvailability.filter(r => r.user_id === currentUser.id).map(r => `${r.slot_date}|${r.slot_time}`)
  );
  ...
  if (legend) {
    legend.innerHTML = usersInData.map(uid => {
      ...
      return `<span style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text-secondary)">
        <span class="avail-legend-dot" style="background:${col}"></span>${escapeHtml(name)}
      </span>`;
  ...
  let html = `<div class="availability-calendar-wrap"><table class="availability-table"><thead><tr>
  ...
  times.forEach(time => {
    ...
    days.forEach(day => {
      ...
      html += `<td>
        <div class="${slotClass}" style="${slotStyle}" onclick="toggleAvailability('${day}','${time}:00','${userColourMap[currentUser.id]||'#00aeef'}')">
          ${dots}${count >= 2 ? `<span class="avail-count">${count}</span>` : ''}
        </div>
      </td>`;
    });
    html += '</tr>';
...
function renderRecommendedSlots(days, times, totalUsers) {
  ...
  days.forEach(day => {
    times.forEach(time => {
      ...
      if (selectors.length >= totalUsers && totalUsers > 0) {
        const dt = new Date(day + 'T12:00:00');
        recommended.push({ day, time, dateStr: dt.toLocaleDateString('en-ZA',{ weekday:'short', day:'numeric', month:'short' }) });
      }
    });
  });
  el.innerHTML = recommended.length === 0
    ...
    : recommended.map(r => `<span class="recommended-slot-chip"><i class="ti ti-star"></i> ${r.dateStr} at ${r.time}</span>`).join('');
}
...
async function toggleAvailability(date, time, colour) {
  ...
  if (myAvailability.has(key)) {
    ...
    await db.from('team_availability').delete()
      .eq('user_id', currentUser.id).eq('slot_date', date).eq('slot_time', time);
  } else {
    myAvailability.add(key);
    await db.from('team_availability').upsert({
      user_id: currentUser.id, user_name: fullName, user_colour: colour,
      slot_date: date, slot_time: time, created_at: new Date().toISOString()
    });
  }
...
async function renderOnboardingSlots() {
  ...
  if (slots.length === 0) {
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:30px;color:var(--text-muted)">
      No onboarding slots available yet.${userRole === 'admin' ? ' Click "Add Slot" to create one.' : ''}
    </div>`;
    return;
  }
  ...
  slots.forEach(slot => {
    ...
    if (isMine) {
      ...
      actionHtml = `<div style="margin-top:10px">
        <button class="btn btn-danger btn-full btn-sm" onclick="confirmCancelOnboarding('${slot.id}','${escapeHtml(slot.booked_by_name||'')}','${slot.slot_date}','${slot.slot_time.slice(0,5)}')">
          <i class="ti ti-x"></i> Cancel
        </button></div>`;
    } else if (isOther && userRole === 'admin') {
      cls        = 'slot-card booked-admin';
    ...
    if (!slot.booked_by_id) {
      actionHtml = `<div style="margin-top:10px">
        <button class="btn btn-primary btn-full btn-sm" onclick="openOnboardingTopicModal('${slot.id}')">
...
async function confirmOnboardingBook() {
  ...
  const { error } = await db.from('onboarding_slots').update({
    booked_by_id: currentUser.id, booked_by_name: fullName,
...
function confirmCancelOnboarding(slotId, name, date, time) {
  document.getElementById('cancelOnboardingText').innerHTML =
    `Are you sure you want to cancel the session for <strong>${escapeHtml(name)}</strong> on <strong>${date} at ${time}</strong>?`;
...
function buildDonutPaths(good, moderate, poor) {
  ...
  if (total === 0) return `<circle cx="55" cy="55" r="40" fill="none" stroke="var(--border)" stroke-width="18"/>
  ...
  const paths = segs.map(seg => {
    ...
    const gap  = circ - dash;
    const path = `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${seg.color}" stroke-width="18"
...
async function downloadStatsImage() {
  ...
  try {
    if (typeof html2canvas !== 'undefined') {
      ...
      alert('Add to your <head>:\n<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"><\/script>');
    }
  } catch (err) { alert('Download failed: ' + err.message); }
}

/* ═══════════════════════════════════════════
   SIGN TAB — DOCUMENT SIGNING
═══════════════════════════════════════════ */
...
/* ── ADMIN: Managers list + assigned docs ── */
async function renderSignAdminView() {
  const { data: managers } = await db.from('profiles').select('*').eq('role','manager').order('department');
  allManagers = managers || [];

  const listEl = document.getElementById('signManagerList');

  if (allManagers.length === 0) {
    listEl.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No managers found. Managers must create accounts first.</p>';
  } else {
    const byDept = {};
    ...
    listEl.innerHTML = Object.entries(byDept).map(([dept, mgrs]) => `
      <div style="margin-bottom:14px">
        ...
        ${mgrs.map(m => {
          ...
          return `
            <div class="sign-manager-card ${selectedSignManagerId === m.id ? 'selected' : ''}" onclick="selectSignManager('${m.id}')">
              <div class="sign-manager-info">
                <div class="applicant-avatar">${getInitials(fullName)}</div>
                <div>
                  <div class="sign-manager-name">${escapeHtml(fullName)}</div>
                  <div class="sign-manager-dept">${escapeHtml(m.email)}</div>
                </div>
              </div>
  ...
  // Render all assigned docs
  await renderAllSignedDocsList();

  // If a manager is already selected, refresh their upload panel
  if (selectedSignManagerId) {
    await renderSignUploadPanel(selectedSignManagerId);
  }
}

function selectSignManager(managerId) {
  selectedSignManagerId = managerId;
  renderSignAdminView();
}

async function renderSignUploadPanel(managerId) {
  const manager = allManagers.find(m => m.id === managerId);
  if (!manager) return;
  const fullName = `${manager.first_name} ${manager.last_name}`.trim() || manager.email;

  const panel = document.getElementById('signUploadPanel');

  // Get docs already assigned to this manager
  const { data: docs } = await db.from('sign_documents').select('*').eq('assigned_to', managerId).order('created_at',{ascending:false});

  panel.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
      <div>
        <div style="font-size:14px;font-weight:600">${escapeHtml(fullName)}</div>
        <div style="font-size:11px;color:var(--text-muted)">${escapeHtml(manager.department || '')}</div>
      </div>
      <button class="btn btn-primary btn-sm" onclick="openAssignDocModal('${managerId}','${escapeHtml(fullName)}')">
        <i class="ti ti-file-upload"></i> Assign Document
      </button>
    </div>
    <div id="signManagerDocsList">
      ${(docs && docs.length > 0) ? '' : '<p style="font-size:12px;color:var(--text-muted)">No documents assigned to this manager yet.</p>'}
    </div>`;

  if (docs && docs.length > 0) {
    const listEl = document.getElementById('signManagerDocsList');
    for (const doc of docs) {
      const { data: sigs } = await db.from('document_signatures').select('*').eq('document_id', doc.id);
      const signed = (sigs || []).length > 0;
      listEl.innerHTML += `
...
/* ── ALL ASSIGNED DOCS (admin overview) ── */
async function renderAllSignedDocsList() {
  ...
  if (!docs || docs.length === 0) {
    el.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No documents have been assigned yet.</p>';
    return;
  }
    ...
    el.innerHTML += `
      <div class="sign-doc-card">
        <div class="flex-between">
          <div>
            <div class="sign-doc-title">${escapeHtml(doc.title)}</div>
            <div class="sign-doc-meta">Assigned to: ${escapeHtml(doc.assigned_name || '—')}</div>
            <div class="sign-doc-meta ${signed ? 'sign-status-signed' : 'sign-status-pending'}">
              <i class="ti ${signed ? 'ti-circle-check' : 'ti-clock'}"></i>
              ${signed ? 'Signed' : 'Pending Signature'}
            </div>
...
function openAssignDocModal(managerId, managerName) {
  selectedSignManagerId = managerId;
  document.getElementById('assignDocManagerName').textContent = managerName;
...
function handleAssignDocUpload(e) {
  ...
  if (pendingAssignFile) {
    document.getElementById('assignDocFileLabel').textContent = `✓ ${pendingAssignFile.name}`;
  }
}
...
async function saveAssignedDocument() {
  ...
  const manager  = allManagers.find(m => m.id === selectedSignManagerId);
  const fullName = manager ? (`${manager.first_name} ${manager.last_name}`.trim() || manager.email) : '';

  const safeName = title.replace(/[^a-zA-Z0-9_-]/g, '_');
  const path     = `signdocs/${selectedSignManagerId}_${Date.now()}_${safeName}.pdf`;

  const { error: uploadErr } = await db.storage.from('signdocs').upload(path, pendingAssignFile, { upsert: true });
  if (uploadErr) { alert('Could not upload document: ' + uploadErr.message); return; }
  ...
  const { error } = await db.from('sign_documents').insert({
    ...
    assigned_to: selectedSignManagerId,
    assigned_name: fullName,
    assigned_dept: manager?.department || '',
    uploaded_by: currentUser.id,
    status: 'pending',
    created_at: new Date().toISOString()
  });

  if (error) { alert('Could not assign document: ' + error.message); return; }

  closeModal('assignDocModal');
  await renderSignAdminView();
  showToast('Document assigned successfully.');
}

/* ── MANAGER: My documents to sign ── */
async function renderSignManagerView() {
  const el = document.getElementById('signMyDocsList');
  if (!el) return;

  const { data: docs } = await db.from('sign_documents').select('*').eq('assigned_to', currentUser.id).order('created_at',{ascending:false});
      ...
      <div class="sign-doc-card">
        <div class="flex-between">
          <div>
            ...
            <div class="sign-doc-meta ${signed ? 'sign-status-signed' : 'sign-status-pending'}">
              <i class="ti ${signed ? 'ti-circle-check' : 'ti-clock'}"></i>
              ${signed ? `Signed on ${new Date(sig.signed_at).toLocaleDateString('en-ZA',{day:'numeric',month:'short',year:'numeric'})}` : 'Awaiting your signature'}
            </div>
          </div>
...
/* ── SIGN DOCUMENT MODAL (live PDF view + signature) ── */
async function openSignDocModal(docId) {
  ...
  const content = document.getElementById('signDocContent');
  content.innerHTML = `
    <p style="font-size:13px;color:var(--text-muted);margin-bottom:10px">${escapeHtml(doc.description || '')}</p>
    <iframe class="sign-pdf-viewer" src="${doc.file_url}"></iframe>
    <div class="signature-pad-wrap">
      ${sig
        ? `<div style="text-align:center">
            <i class="ti ti-circle-check" style="font-size:28px;color:var(--success);display:block;margin-bottom:8px"></i>
            <div style="font-size:13px;font-weight:600">Signed by ${escapeHtml(sig.signer_name)}</div>
            <div style="font-size:11px;color:var(--text-muted)">${new Date(sig.signed_at).toLocaleString('en-ZA')}</div>
          </div>`
        : `<p style="font-size:13px;color:var(--text-secondary);margin-bottom:10px">
            By clicking "Sign Document" below, you confirm that you have reviewed this document
            and agree to its contents. Your name and the current date/time will be recorded as your electronic signature.
          </p>
          <div style="font-family:'DM Serif Display',serif;font-size:20px;color:var(--brand-dark);padding:12px;border:1px dashed var(--border-strong);border-radius:8px">
            ${escapeHtml(`${currentProfile.first_name} ${currentProfile.last_name}`.trim())}
          </div>`}
    </div>
    <div class="signature-list" id="docSignatureList"></div>`;

  // Show/hide sign button
  const signBtn = document.getElementById('signDocBtn');
  signBtn.style.display = sig ? 'none' : 'inline-flex';

  // Show all signatures for this doc (for context)
  const { data: allSigs } = await db.from('document_signatures').select('*').eq('document_id', docId);
  const sigListEl = document.getElementById('docSignatureList');
  if (allSigs && allSigs.length > 0) {
    sigListEl.innerHTML = `<div class="info-label" style="margin-bottom:6px">Signed By</div>` +
      allSigs.map(s => `
        <div class="signature-row">
          <span>${escapeHtml(s.signer_name)}</span>
          <span style="color:var(--text-muted)">${new Date(s.signed_at).toLocaleDateString('en-ZA',{day:'numeric',month:'short',year:'numeric'})}</span>
        </div>`).join('');
  }
...
async function signDocument() {
  ...
  if (error) { alert('Could not sign document: ' + error.message); return; }
...
function checkForStatusChangeNotification(currentStatus) {
  const lastSeen = localStorage.getItem('lastSeenStatus_' + currentUser.id);
  if (lastSeen && lastSeen !== currentStatus && stageMessages[currentStatus]) {
    showCongratsPopup(currentStatus);
  }
  localStorage.setItem('lastSeenStatus_' + currentUser.id, currentStatus);
}