/* ===========================================
   COMMUNICARE HRIS — app.js  (v11 - + Interviews)
=========================================== */
...
/* --- Interviews feature state --- */
let currentInterviewBookingId = null;
let currentInterviewEval      = null;
let currentInterviewTemplate  = null;
...
/* Default interview guide template — 3 sections × 7 questions, fully editable by admin */
const DEFAULT_INTERVIEW_TEMPLATE = [
  {
    id: 's1', title: 'Section 1 — Background & Experience',
    questions: [
      { id: 'q1', text: 'Walk us through your current role and key responsibilities.' },
      { id: 'q2', text: 'What relevant experience do you have for this role?' },
      { id: 'q3', text: 'Describe a significant achievement in your career so far.' },
      { id: 'q4', text: 'Why are you interested in this position at Communicare?' },
      { id: 'q5', text: 'What do you know about our organisation and this role?' },
      { id: 'q6', text: 'Describe your ideal working environment.' },
      { id: 'q7', text: 'What are your salary expectations and notice period?' }
    ]
  },
  {
    id: 's2', title: 'Section 2 — Competency & Skills',
    questions: [
      { id: 'q1', text: 'Describe a time you solved a complex problem at work.' },
      { id: 'q2', text: 'How do you prioritise tasks when under pressure?' },
      { id: 'q3', text: 'Give an example of working effectively in a team.' },
      { id: 'q4', text: 'How do you handle conflict with a colleague or manager?' },
      { id: 'q5', text: 'What technical or functional skills make you suited to this role?' },
      { id: 'q6', text: 'Describe a mistake you made and what you learned from it.' },
      { id: 'q7', text: 'How do you keep your skills and knowledge up to date?' }
    ]
  },
  {
    id: 's3', title: 'Section 3 — Culture Fit & Motivation',
    questions: [
      { id: 'q1', text: 'What values are important to you in a workplace?' },
      { id: 'q2', text: 'How would your colleagues describe you?' },
      { id: 'q3', text: 'What motivates you to do your best work?' },
      { id: 'q4', text: 'Where do you see yourself in 3–5 years?' },
      { id: 'q5', text: 'How do you handle change and ambiguity?' },
      { id: 'q6', text: 'What questions do you have for the panel?' },
      { id: 'q7', text: 'Any final comments or concerns from the interview panel?' }
    ]
  }
];
...
const navConfig = {
  admin: [
    ...
    { id: 'interviews',  icon: 'ti-clipboard-text',    label: 'Interviews' },
    { section: 'Documents' },
...
function showTab(id, el) {
  ...
  if (id === 'interviews')    renderInterviewsTab();
}
...
async function deleteAccount(profileId, fullName) {
  ...
  if (error) {
    ...
    await Promise.all([
      ...
      db.from('interview_evaluations').delete().eq('interviewer_id', profileId),
    ]);
...
/* ===========================================
   EMAIL — fire and forget, never blocks UI
=========================================== */
...
async function informManagerOfShortlist() {
  ...
  showToast('Shortlist saved. Manager will be notified.');
...
async function finaliseApplicationStatuses() {
  ...
  if (blockers.length > 0) { alert(`${blockers.length} candidate(s) marked "Don't Continue".`); return; }
...
async function openManagerCandidateModal(appId) {
  ...
  content.innerHTML = `
    ...
    <div class="card" style="padding:16px">
      <div class="card-title"><i class="ti ti-gavel"></i> Decision</div>
...
async function renderCandidateBookingSlots() {
  ...
  if (myBooking) {
    ...
    if (!grid) return;
    if (available.length === 0) { grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:30px;color:var(--text-muted)">No slots available yet.</div>`; return; }
    grid.innerHTML = '';
...
/* ===========================================
   AVAILABILITY — only invited users see it
=========================================== */
...
async function renderAdminInterviewVacancies() {
  ...
  let html = `
    <div class="page-header" style="padding-top:16px">
      ...
      <p>Manage interview slots per vacancy.</p>
    </div>
    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px">`;
...
async function openInterviewSlotsForVacancy(vacId, vacTitle) {
  ...
  detail.innerHTML = `
    <div class="card flex-between" style="margin-bottom:16px">
      <div><div class="card-title" style="font-size:14px;font-weight:600"><i class="ti ti-video"></i> ${escapeHtml(vacTitle)} — Interview Slots</div></div>
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div>
        ${(shortlisted || []).length === 0
          ? '<p style="font-size:13px;color:var(--text-muted)">No shortlisted candidates yet.</p>'
          : (shortlisted || []).map(a => `
              <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border)">
                ...
                <button class="btn btn-secondary btn-sm" onclick="inviteCandidateToBookSlot('${a.candidate_email}','${escapeHtml(a.candidate_name || '')}','${escapeHtml(vacTitle)}')"><i class="ti ti-mail"></i> Invite</button>
              </div>`).join('')}
      </div>
    </div>
...
function renderAdminSlotCards(slots, vacId) {
  if (slots.length === 0) return `<div style="grid-column:1/-1;text-align:center;padding:30px;color:var(--text-muted)">No slots yet.</div>`;
...
/* ===========================================
   INTERVIEWS TAB (new feature)
=========================================== */
async function renderInterviewsTab() {
  const el = document.getElementById('interviewBlocksList'); if (!el) return;
  const templateBtn = document.getElementById('manageTemplateBtn');
  if (templateBtn) templateBtn.style.display = userRole === 'admin' ? 'inline-flex' : 'none';

  el.innerHTML = '<div style="padding:30px;text-align:center;color:var(--text-muted)">Loading interviews…</div>';

  const { data: allVacs } = await db.from('vacancies').select('*');
  let allowedVacIds = null;

  if (userRole !== 'admin') {
    const { data: inviteRows } = await db.from('calendar_invites').select('vacancy_id,invitee_id');
    const ids = new Set();
    (inviteRows || []).forEach(row => {
      const invitees = safeParseJSON(row.invitee_id, []);
      if (invitees.includes(currentUser.id)) ids.add(String(row.vacancy_id));
    });
    allowedVacIds = ids;
  }

  const { data: bookings } = await db.from('bookings')
    .select('*').not('booked_by_email', 'is', null)
    .order('slot_date', { ascending: true }).order('slot_time', { ascending: true });

  let list = (bookings || []).filter(b => b.vacancy_id);
  if (allowedVacIds) list = list.filter(b => allowedVacIds.has(String(b.vacancy_id)));

  if (list.length === 0) {
    el.innerHTML = `<div class="card" style="text-align:center;padding:40px">
      <i class="ti ti-calendar-off" style="font-size:36px;color:var(--text-muted);display:block;margin-bottom:12px"></i>
      <div style="font-size:14px;color:var(--text-muted)">${userRole === 'admin' ? 'No interviews booked yet.' : 'No interviews to display yet. You will see candidates here once you are invited to their panel and they have booked a slot.'}</div>
    </div>`;
    return;
  }

  const { data: myEvals } = await db.from('interview_evaluations').select('booking_id').eq('interviewer_id', currentUser.id);
  const startedSet = new Set((myEvals || []).map(e => String(e.booking_id)));

  el.innerHTML = '<div class="interview-grid">' + list.map(b => {
    const vac     = (allVacs || []).find(v => String(v.id) === String(b.vacancy_id));
    const dateStr = new Date(`${b.slot_date}T${b.slot_time}`).toLocaleDateString('en-ZA', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' });
    const timeStr = b.slot_time.slice(0,5);
    const started = startedSet.has(String(b.id));
    return `<div class="interview-block" onclick="openInterviewGuide('${b.id}')">
      <div class="interview-block-top">
        <div class="applicant-avatar" style="width:44px;height:44px;font-size:15px">${getInitials(b.booked_by_name || 'CA')}</div>
        <div style="flex:1;min-width:0">
          <div class="interview-block-name">${escapeHtml(b.booked_by_name || 'Candidate')}</div>
          <div class="interview-block-role">${escapeHtml(vac?.title || 'Unknown Role')}</div>
        </div>
        ${started ? '<span class="status-pill green" style="flex-shrink:0"><i class="ti ti-notes"></i></span>' : ''}
      </div>
      <div class="interview-block-meta">
        <span><i class="ti ti-calendar"></i> ${dateStr}</span>
        <span><i class="ti ti-clock"></i> ${timeStr}</span>
      </div>
    </div>`;
  }).join('') + '</div>';
}

async function openInterviewGuide(bookingId) {
  const { data: booking } = await db.from('bookings').select('*').eq('id', bookingId).single();
  if (!booking) { alert('Booking not found.'); return; }
  const { data: vac }     = await db.from('vacancies').select('*').eq('id', booking.vacancy_id).maybeSingle();
  const { data: profile } = await db.from('profiles').select('*').eq('email', booking.booked_by_email).maybeSingle();

  currentInterviewBookingId = bookingId;
  currentInterviewTemplate  = await getInterviewTemplate();

  const { data: existingEval } = await db.from('interview_evaluations').select('*').eq('booking_id', bookingId).eq('interviewer_id', currentUser.id).maybeSingle();

  currentInterviewEval = existingEval || {
    booking_id: bookingId, vacancy_id: booking.vacancy_id,
    candidate_name: booking.booked_by_name, candidate_email: booking.booked_by_email,
    interviewer_id: currentUser.id,
    interviewer_name: `${currentProfile.first_name} ${currentProfile.last_name}`.trim(),
    answers: {}
  };
  currentInterviewEval.answers = safeParseJSON(currentInterviewEval.answers, {}) || {};

  document.getElementById('iguideCandidateName').textContent = booking.booked_by_name || 'Candidate';
  document.getElementById('iguideRole').textContent = vac?.title ? `Applied for: ${vac.title}` : '';
  document.getElementById('iguideMeta').textContent =
    `${new Date(`${booking.slot_date}T${booking.slot_time}`).toLocaleDateString('en-ZA', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })} · ${booking.slot_time.slice(0,5)}${profile?.email ? ' · ' + profile.email : ''}`;
  document.getElementById('iguideSaveIndicator').textContent = '';

  renderInterviewGuideBody();
  document.getElementById('interviewGuideModal').classList.add('open');
}

function renderInterviewGuideBody() {
  const toc  = document.getElementById('iguideToc');
  const body = document.getElementById('iguideScrollBody');
  if (!toc || !body || !currentInterviewTemplate) return;
  const sections = currentInterviewTemplate.sections || [];

  toc.innerHTML = sections.map((s, si) =>
    `<div class="iguide-toc-item" onclick="scrollToSection('${s.id}')"><span class="iguide-toc-num">${si+1}</span>${escapeHtml(s.title)}</div>`
  ).join('') + (userRole === 'admin' ? `<button class="btn btn-secondary btn-sm btn-full" style="margin-top:12px" onclick="openInterviewTemplateModal()"><i class="ti ti-settings"></i> Edit Template</button>` : '');

  body.innerHTML = sections.map((s, si) => `
    <div class="iguide-section" id="iguide_sec_${s.id}">
      <div class="iguide-section-title"><span class="iguide-section-num">${si+1}</span>${escapeHtml(s.title)}</div>
      ${(s.questions || []).map(q => {
        const key = `${s.id}_${q.id}`;
        const ans = currentInterviewEval.answers[key] || {};
        return `<div class="iguide-question" id="iguide_q_${s.id}_${q.id}">
          <div class="iguide-question-text">${escapeHtml(q.text)}</div>
          <div class="iguide-question-row">
            <textarea class="iguide-notes" placeholder="Notes…" oninput="saveInterviewAnswer('${s.id}','${q.id}','notes',this.value)">${escapeHtml(ans.notes || '')}</textarea>
            <select class="iguide-rating" onchange="saveInterviewAnswer('${s.id}','${q.id}','rating',this.value)">
              <option value="">Rate</option>
              ${[1,2,3,4,5].map(n => `<option value="${n}" ${String(ans.rating) === String(n) ? 'selected' : ''}>${n}</option>`).join('')}
            </select>
          </div>
        </div>`;
      }).join('')}
...
function scrollToSection(sectionId) {
  const el = document.getElementById('iguide_sec_' + sectionId);
  if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function scrollToNextQuestion() {
  const body = document.getElementById('iguideScrollBody'); if (!body) return;
  const bodyRect = body.getBoundingClientRect();
  const questions = [...body.querySelectorAll('.iguide-question')];
  for (const q of questions) {
    const r = q.getBoundingClientRect();
    if (r.top > bodyRect.top + 40) { q.scrollIntoView({ behavior: 'smooth', block: 'start' }); return; }
  }
  if (questions.length > 0) questions[questions.length - 1].scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function saveInterviewAnswer(sectionId, questionId, field, value) {
  const key = `${sectionId}_${questionId}`;
  if (!currentInterviewEval.answers[key]) currentInterviewEval.answers[key] = {};
  currentInterviewEval.answers[key][field] = value;
  const indicator = document.getElementById('iguideSaveIndicator');
  if (indicator) indicator.textContent = 'Saving…';
  clearTimeout(interviewSaveTimeout);
  interviewSaveTimeout = setTimeout(persistInterviewEval, 700);
}

async function persistInterviewEval() {
  if (!currentInterviewEval) return;
  const payload = {
    booking_id:       currentInterviewEval.booking_id,
    vacancy_id:        currentInterviewEval.vacancy_id,
    candidate_name:    currentInterviewEval.candidate_name,
    candidate_email:   currentInterviewEval.candidate_email,
    interviewer_id:    currentInterviewEval.interviewer_id,
    interviewer_name:  currentInterviewEval.interviewer_name,
    answers:           JSON.stringify(currentInterviewEval.answers || {}),
    updated_at:        new Date().toISOString()
  };
  try {
    if (currentInterviewEval.id) {
      await db.from('interview_evaluations').update(payload).eq('id', currentInterviewEval.id);
    } else {
      payload.created_at = new Date().toISOString();
      const { data, error } = await db.from('interview_evaluations').upsert(payload, { onConflict: 'booking_id,interviewer_id' }).select().single();
      if (!error && data) currentInterviewEval.id = data.id;
    }
    const indicator = document.getElementById('iguideSaveIndicator');
...
async function closeInterviewGuideModal() {
  clearTimeout(interviewSaveTimeout);
  await persistInterviewEval();
  closeModal('interviewGuideModal');
  await renderInterviewsTab();
}

async function getInterviewTemplate() {
  const { data } = await db.from('interview_templates').select('*').eq('id', 'default').maybeSingle();
  if (data && data.sections) {
    return { id: 'default', sections: safeParseJSON(data.sections, DEFAULT_INTERVIEW_TEMPLATE) };
  }
  await db.from('interview_templates').upsert({ id: 'default', sections: JSON.stringify(DEFAULT_INTERVIEW_TEMPLATE), updated_at: new Date().toISOString() });
  return { id: 'default', sections: JSON.parse(JSON.stringify(DEFAULT_INTERVIEW_TEMPLATE)) };
}

function genTemplateId(prefix) { return prefix + '_' + Math.random().toString(36).slice(2, 9); }

async function openInterviewTemplateModal() {
  if (userRole !== 'admin') return;
  currentInterviewTemplate = await getInterviewTemplate();
  renderTemplateEditor();
  document.getElementById('interviewTemplateModal').classList.add('open');
}

function renderTemplateEditor() {
  const el = document.getElementById('templateEditorBody'); if (!el || !currentInterviewTemplate) return;
  const sections = currentInterviewTemplate.sections || [];
  el.innerHTML = sections.map(s => `
    <div class="template-section-block">
      <div class="template-section-head">
        <input type="text" value="${escapeHtml(s.title)}" placeholder="Section title" onchange="updateTemplateSectionTitle('${s.id}',this.value)"/>
        <button class="btn btn-danger btn-sm" onclick="removeTemplateSection('${s.id}')"><i class="ti ti-trash"></i></button>
      </div>
      <div class="template-question-list">
        ${(s.questions || []).map(q => `
          <div class="template-question-row">
            <input type="text" value="${escapeHtml(q.text)}" placeholder="Question text" onchange="updateTemplateQuestionText('${s.id}','${q.id}',this.value)"/>
            <button class="job-btn del" onclick="removeTemplateQuestion('${s.id}','${q.id}')"><i class="ti ti-x"></i></button>
          </div>`).join('')}
      </div>
      <button class="btn btn-secondary btn-sm" onclick="addTemplateQuestion('${s.id}')"><i class="ti ti-plus"></i> Add Question</button>
    </div>`).join('') + `<button class="btn btn-purple btn-full" onclick="addTemplateSection()"><i class="ti ti-plus"></i> Add Section</button>`;
}

function addTemplateSection() {
  currentInterviewTemplate.sections.push({ id: genTemplateId('s'), title: 'New Section', questions: [{ id: genTemplateId('q'), text: 'New question…' }] });
  renderTemplateEditor();
}
function removeTemplateSection(sid) {
  if (!confirm('Remove this section and all its questions?')) return;
  currentInterviewTemplate.sections = currentInterviewTemplate.sections.filter(s => s.id !== sid);
  renderTemplateEditor();
}
function updateTemplateSectionTitle(sid, val) {
  const s = currentInterviewTemplate.sections.find(s => s.id === sid); if (s) s.title = val;
}
function addTemplateQuestion(sid) {
  const s = currentInterviewTemplate.sections.find(s => s.id === sid); if (!s) return;
  s.questions.push({ id: genTemplateId('q'), text: 'New question…' });
  renderTemplateEditor();
}
function removeTemplateQuestion(sid, qid) {
  const s = currentInterviewTemplate.sections.find(s => s.id === sid); if (!s) return;
  s.questions = s.questions.filter(q => q.id !== qid);
  renderTemplateEditor();
}
function updateTemplateQuestionText(sid, qid, val) {
  const s = currentInterviewTemplate.sections.find(s => s.id === sid); if (!s) return;
  const q = s.questions.find(q => q.id === qid); if (q) q.text = val;
}

async function saveInterviewTemplate() {
  await db.from('interview_templates').upsert({
    id: 'default', sections: JSON.stringify(currentInterviewTemplate.sections),
    updated_at: new Date().toISOString(), updated_by: currentUser.id
  });
  showToast('Interview template saved.');
  closeModal('interviewTemplateModal');
  if (document.getElementById('interviewGuideModal').classList.contains('open')) renderInterviewGuideBody();
}
...
/* ===========================================
   RETRIEVE DOCUMENT — screenshot approach
=========================================== */
async function retrieveDocument(docId) {
  ...
  if (!confirm(`${doc.title}\n\n${signedCount}/${totalCount} signed · ${sigs.length} signature placement${sigs.length !== 1 ? 's' : ''}\n\nThis will open the document with all signatures visible. Once it loads, click "Download Signed PDF" to save it permanently.`)) return;

  await openSigningModal(docId, null, false, true);

  showToast('Document loading — wait a moment then click "Download Signed PDF" in the left panel.', 'info');

  await db.from('sign_documents').update({ retrieved: true, retrieved_at: new Date().toISOString() }).eq('id', docId);

  setTimeout(() => {
    const leftPanel = document.querySelector('.sign-left-panel');
    if (!leftPanel) return;
    const existing = document.getElementById('signedDownloadBtn');
    if (existing) existing.remove();
    const dlWrap = document.createElement('div');
    dlWrap.id = 'signedDownloadBtn';
    dlWrap.style.cssText = 'padding:12px 0;border-top:1px solid var(--border);margin-top:8px';
    dlWrap.innerHTML = `
      <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px;line-height:1.5">
        <i class="ti ti-info-circle"></i> Wait for the document to fully load, then click below.
      </div>
      <button class="btn btn-success btn-full" style="justify-content:center" onclick="captureSignedDocument('${escapeHtml(doc.title)}','${escapeHtml(doc.file_name || 'document.pdf')}')">
        <i class="ti ti-download"></i> Download Signed PDF
      </button>`;
    leftPanel.appendChild(dlWrap);
  }, 600);

  await renderSignAdminView();
}

async function captureSignedDocument(docTitle, originalFileName) {
  const wrap = document.getElementById('signFrameWrap');
  if (!wrap) { alert('Document not open. Please open it first.'); return; }
  ...
  const btn = document.querySelector('#signedDownloadBtn button');
  if (btn) { btn.disabled = true; btn.innerHTML = '<i class="ti ti-loader"></i> Capturing… please wait'; }
  showToast('Capturing document with signatures — this takes a few seconds…', 'info');

  try {
    const rightPanel = document.querySelector('.sign-right-panel');
    ...
    const safeName = (originalFileName || docTitle || 'document').replace('.pdf','').replace(/[^a-zA-Z0-9_\- ]/g,'_');
    link.href     = url;
    link.download = safeName + '_signed.pdf';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    setTimeout(() => URL.revokeObjectURL(url), 10000);

    showToast('✓ Signed PDF downloaded — signatures permanently embedded!');
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="ti ti-download"></i> Download Signed PDF'; }

  } catch (err) {
    console.error('Capture error:', err);
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="ti ti-download"></i> Download Signed PDF'; }
    alert('Capture failed: ' + err.message + '\n\nMake sure the document has fully loaded, then try again.');
  }
}
      ...
      <div class="sign-doc-card">
        <div class="flex-between">
          <div>
            ...
            ${isRetrieved ? `<div class="sign-doc-meta" style="color:var(--warning)"><i class="ti ti-lock"></i> Retrieved — read only</div>` : ''}
          </div>
...
document.addEventListener('click', e => {
  if (e.target.classList.contains('modal-overlay') && e.target.id !== 'signDocModal' && e.target.id !== 'interviewGuideModal') {
