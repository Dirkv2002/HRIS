/* ===========================================
   COMMUNICARE HRIS — app.js  (v10 - FINAL)
=========================================== */
...
async function informManagerOfShortlist() {
  ...
  // Single delete + bulk insert = fast
...
/* ===========================================
   RETRIEVE DOCUMENT
   Uses html2canvas to screenshot the rendered
   document + overlay together, then wraps it
   into a PDF — bypasses CORS completely.
...
async function retrieveDocument(docId) {
  ...
  if (!confirm(`${doc.title}\n\n${signedCount}/${totalCount} signed · ${sigs.length} signature placement${sigs.length !== 1 ? 's' : ''}\n\nThis will open the document with all signatures visible so you can download it as a permanent PDF.`)) return;

  // Open in read-only admin view so all sigs are visible
  await openSigningModal(docId, null, false, true);

  showToast('Document open — wait for it to load fully, then click "Download Signed PDF" in the left panel.', 'info');

  // Mark as retrieved
  await db.from('sign_documents').update({ retrieved: true, retrieved_at: new Date().toISOString() }).eq('id', docId);

  // Inject the download button into the left panel
  setTimeout(() => {
    ...
    dlWrap.innerHTML = `
      <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px;line-height:1.6">
        <i class="ti ti-info-circle"></i> Scroll through the document to confirm all signatures are visible, then click the button below.
      </div>
...
/* Captures the visible iframe + overlay as a single PDF page */
async function captureSignedDocument(docTitle, originalFileName) {
  const wrap = document.getElementById('signFrameWrap');
  if (!wrap) { alert('Document not visible — please open it first.'); return; }

  // Check libraries are loaded
  if (typeof html2canvas === 'undefined') {
    alert('html2canvas not loaded. Please add this to your HTML <head>:\n<script src="https://unpkg.com/html2canvas@1.4.1/dist/html2canvas.min.js"><\/script>');
    return;
  }
  if (typeof PDFLib === 'undefined') {
    alert('pdf-lib not loaded. Please add this to your HTML <head>:\n<script src="https://unpkg.com/pdf-lib@1.17.1/dist/pdf-lib.min.js"><\/script>');
    return;
  }
  ...
  showToast('Capturing signatures — this takes a few seconds…', 'info');

  try {
    // Scroll right panel to top so we capture from the beginning
    const rightPanel = document.querySelector('.sign-right-panel');
    if (rightPanel) rightPanel.scrollTop = 0;
    await new Promise(r => setTimeout(r, 1000)); // let iframe settle

    // Screenshot the wrap div — captures iframe content + overlay signatures together
    ...
    // Convert canvas → JPEG → PDF page via pdf-lib
    const { PDFDocument } = PDFLib;
    const pdfDoc  = await PDFDocument.create();

    // Canvas is 2x scaled so divide dimensions back down
    const pageW = canvas.width  / 2;
    const pageH = canvas.height / 2;
    const page  = pdfDoc.addPage([pageW, pageH]);

    // Embed as JPEG (smaller than PNG)
    ...
    showToast('✓ Signed PDF downloaded — signatures are permanently embedded!');
    ...
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="ti ti-download"></i> Download Signed PDF'; }
    alert('Could not capture document: ' + err.message + '\n\nTry scrolling to the top of the document first, then click Download again.');
  }
}
...
/* ===========================================
   SIGNING MODAL
   - Adobe-style: scrollable dark panel + document
   - Live polling every 3 seconds (free, no Realtime)
   - All signatures visible to all assignees
   - Drag fixed: pure percentage delta in wrap space
=========================================== */
    ...
    <div class="sign-layout">
      <div class="sign-left-panel">
        ...
        <div style="font-size:10px;color:var(--text-muted);background:var(--surface-2);padding:8px;border-radius:6px;line-height:1.5;margin-top:4px">
          <i class="ti ti-refresh"></i> Updates every 3 seconds — all assignees see the same signatures live.
        </div>
      ...
      <div class="sign-right-panel">
        <!--
          signFrameWrap is the SINGLE coordinate reference for all signature positions.
          The overlay lives INSIDE the wrap so coordinates are always consistent.
          This is the fix for the "signatures run away" drag bug.
        -->
...
/* Click to place — coords relative to signFrameWrap */
...
/* ===========================================
   DRAG FIX — pure percentage delta
   
   Both mouse position and signature start position
   are captured in % space at drag start.
   The delta is computed entirely in % space.
   No pixel/percent mixing = no "running away".
=========================================== */
function sigDragStart(e) {
  ...
  // Lock the wrap rect ONCE at drag start — never re-query during drag
  const wrapRect = wrap.getBoundingClientRect();

  // Mouse start position as % of wrap
  ...
  // Signature start position in % (from DB/state)
...
/* Bulk insert + fire-and-forget emails = instant */
async function confirmAssignDocument() {
  ...
  // Delete all then bulk insert in a single call
