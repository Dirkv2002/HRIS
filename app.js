...
async function deleteAccount(profileId, fullName) {
  ...
  if (error) {
    ...
    await db.from('candidate_assessments').delete().eq('candidate_id', profileId);
    await db.from('document_signatures').delete().eq('signer_id', profileId);
    await db.from('document_assignees').delete().eq('manager_id', profileId);
    await db.from('team_availability').delete().eq('user_id', profileId);
    await db.from('manager_reviews').delete().eq('manager_id', profileId);
    await db.from('applications').delete().eq('user_id', profileId);
    await db.from('applications').delete().eq('candidate_email', email);
    await db.from('shortlists').delete().eq('candidate_email', email);
    await db.from('profiles').delete().eq('id', profileId);
    showToast(`${fullName} profile removed. Note: auth login may still exist.`, 'info');
  } else {
    showToast(`${fullName} has been fully deleted.`);
  }
...
async function sendRejectionEmail(toEmail, n, t)               { await sendEmail(toEmail, `Your application for ${t} — Communicare`, `Dear ${n},\n\nThank you for applying for ${t} at Communicare. After careful consideration, we will not be moving forward with your application at this time.\n\nWe wish you every success.\n\nWith warm regards,\nCommunicare Human Capital Team`); }
async function sendStatusUpdateEmail(toEmail, n, t, s)         { await sendEmail(toEmail, `Application update — ${t}`, `Dear ${n},\n\nYour application for ${t} has been updated to: ${s}.\n\nKind regards,\nCommunicare Human Capital Team`); }
async function sendInterviewConfirmationEmail(toEmail, n, d, t){ await sendEmail(toEmail, `Interview Booking Confirmed — Communicare`, `Dear ${n},\n\nYour interview has been booked for ${d} at ${t} (30 minutes).\n\nKind regards,\nCommunicare Human Capital Team`); }
async function sendInterviewCancellationEmail(toEmail, n, d, t){ await sendEmail(toEmail, `Interview Cancelled — Communicare`, `Dear ${n},\n\nYour interview for ${d} at ${t} has been cancelled.\n\nKind regards,\nCommunicare Human Capital Team`); }
async function sendManagerShortlistEmail(email, n, t)          { await sendEmail(email, `Shortlist ready for review — ${t}`, `Dear ${n},\n\nA shortlist for "${t}" has been submitted for your review.\n\nKind regards,\nCommunicare Human Capital Team`); }
async function sendManagerDocumentEmail(email, n, t)           { await sendEmail(email, `Document awaiting your signature — ${t}`, `Dear ${n},\n\nA document "${t}" has been assigned to you for signing. Please log in and go to the Sign tab.\n\nKind regards,\nCommunicare Human Capital Team`); }
async function sendCandidateInterviewInviteEmail(email, n, t)  { await sendEmail(email, `Interview invitation — ${t}`, `Dear ${n},\n\nCongratulations! You have been shortlisted for an interview for "${t}". Please log in and go to the Bookings tab to select your preferred slot.\n\nKind regards,\nCommunicare Human Capital Team`); }
...
async function renderAvailabilityVacancies() {
  ...
  let html = `
    <div class="page-header" style="padding-top:16px">
      ...
      <p>Select a vacancy to view and mark your availability.</p>
    </div>
  ...
  if (list.length === 0) {
    html += `<div class="card" style="text-align:center;padding:40px">
      ...
      <div style="font-size:14px;color:var(--text-muted)">${userRole === 'admin' ? 'No vacancies found. Create one first.' : 'No vacancy calendars have been shared with you yet.'}</div>
    </div>`;
  } else {
    html += list.map(v => `
  ...
  detail.innerHTML = `
    <div class="card flex-between" style="margin-bottom:16px">
      <div>
        ...
        <p style="font-size:13px;color:var(--text-muted)">Add slots below.</p>
      </div>
...
async function renderOnboardingSlots() {
  ...
  if (list.length === 0) {
    html += `<div class="card" style="text-align:center;padding:40px">
      <i class="ti ti-calendar-off" style="font-size:36px;color:var(--text-muted);display:block;margin-bottom:12px"></i>
      <div style="font-size:14px;color:var(--text-muted)">${userRole === 'admin' ? 'No vacancies found. Create one first.' : 'No onboarding sessions shared with you yet.'}</div>
    </div>`;
  } else {
    html += list.map(v => `
...
async function openOnboardingSlotsForVacancy(vacId, vacTitle) {
  ...
  let slotsHtml = '';
  if (slots.length === 0) {
    slotsHtml = `<div style="text-align:center;padding:30px;color:var(--text-muted)">No onboarding slots yet.${userRole === 'admin' ? ' Click "Add Slot".' : ''}</div>`;
  } else {
...
async function confirmOnboardingBook() {
  ...
  const meta   = typeof pendingOnboardingSlotId === 'object' ? pendingOnboardingSlotId : {};
  const slotId = meta.slotId || pendingOnboardingSlotId; if (!slotId) return;
        ...
        <div class="flex-between">
          <div>
            ...
            <div class="sign-doc-meta ${allSigned ? 'sign-status-signed' : 'sign-status-pending'}">
              ...
              ${signedCount}/${totalCount} assignees signed · ${sigCount} signature${sigCount !== 1 ? 's' : ''} placed
            </div>
            ${doc.retrieved ? `<div class="sign-doc-meta" style="color:var(--warning)"><i class="ti ti-lock"></i> Retrieved ${new Date(doc.retrieved_at).toLocaleDateString('en-ZA')}</div>` : ''}
          </div>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
            ...
            <button class="btn btn-primary btn-sm"   onclick="openSigningModal('${doc.id}',null,false,${!!doc.retrieved})"><i class="ti ti-eye"></i> View Doc</button>
...
async function renderManagerSignStatus() {
  ...
  for (const a of assignees) {
    const doc = docMap[a.document_id];
        ...
        <div class="flex-between">
          <div>
            ...
            <div class="sign-doc-meta ${a.signed ? 'sign-status-signed' : 'sign-status-pending'}">
              ...
              ${a.signed ? 'Signed' : 'Awaiting'}${sigCount > 0 ? ` · ${sigCount} placement${sigCount !== 1 ? 's' : ''}` : ''}
            </div>
...
async function viewDocumentSignatures(docId, docTitle) {
  const modal   = document.getElementById('candidateProfileModal');
  const content = document.getElementById('cpModalContent');
  const nameEl  = document.getElementById('cpModalName');
  nameEl.textContent = docTitle + ' — Signatures';
  content.innerHTML  = '<div style="text-align:center;padding:20px;color:var(--text-muted)">Loading...</div>';
  modal.classList.add('open');

  const { data: assignees } = await db.from('document_assignees').select('*').eq('document_id', docId);
  const { data: sigs }      = await db.from('document_signatures').select('*').eq('document_id', docId);

  const sigsByUser = {};
  (sigs || []).forEach(s => {
    if (!sigsByUser[s.signer_id]) sigsByUser[s.signer_id] = [];
    sigsByUser[s.signer_id].push(s);
  });

  if (!assignees || assignees.length === 0) {
    content.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No one assigned yet.</p>';
    return;
  }

  content.innerHTML = `
    <div class="card-title" style="margin-bottom:12px"><i class="ti ti-users"></i> Assignee Signatures</div>
    ${assignees.map(a => {
      const userSigs = sigsByUser[a.manager_id] || [];
      return `
        <div style="padding:12px 0;border-bottom:1px solid var(--border)">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
            <div>
              <div style="font-size:13px;font-weight:600">${escapeHtml(a.manager_name || a.manager_email || '—')}</div>
              <div style="font-size:11px;color:var(--text-muted)">${escapeHtml(a.manager_email || '')}</div>
            </div>
            <span class="status-pill ${a.signed ? 'green' : 'yellow'}">
              <i class="ti ${a.signed ? 'ti-circle-check' : 'ti-clock'}"></i>
              ${a.signed ? 'Signed' : 'Pending'}
            </span>
          </div>
          ${userSigs.length === 0
            ? '<div style="font-size:12px;color:var(--text-muted);font-style:italic">No placements yet.</div>'
            : `<div style="font-size:12px;color:var(--text-muted);margin-bottom:6px">${userSigs.length} placement${userSigs.length !== 1 ? 's' : ''}</div>
               <div style="display:flex;gap:6px;flex-wrap:wrap">
                 ${userSigs.map(s => s.signature_type === 'draw'
                   ? `<img src="${s.signature_data}" style="height:32px;object-fit:contain;border:1px solid var(--border);border-radius:4px;background:white;padding:2px"/>`
                   : `<span style="font-family:'Dancing Script',cursive;font-size:18px;color:#000;border:1px solid var(--border);border-radius:4px;padding:2px 8px;background:white">${escapeHtml(s.signature_data)}</span>`
                 ).join('')}
               </div>`
          }
        </div>`;
...
async function retrieveDocument(docId) {
  ...
  const msg = `${doc.title}\n\n${signedCount}/${totalCount} assignees signed\n${sigCount} signature placements in database\n\nDownload the original PDF now?`;
  if (!confirm(msg)) return;

  const link = document.createElement('a');
  link.href = doc.file_url; link.target = '_blank'; link.download = doc.file_name || 'document.pdf'; link.click();
...
/* ===========================================
   ...
   - ALL signatures from ALL users visible (live doc)
   - Black signatures, no underline
   - Polling refreshes signatures every 5 seconds
=========================================== */
async function openSigningModal(docId, assigneeId, alreadySigned, isRetrieved) {
  ...
  if (signPollingInterval) { clearInterval(signPollingInterval); signPollingInterval = null; }

  const { data: doc } = await db.from('sign_documents').select('*').eq('id', docId).single();
  if (!doc) return;

  // Load ALL signatures for this document (from all users)
  ...
  // Admin viewing: read only. Manager/employee: can edit if not retrieved
  const isAdmin  = userRole === 'admin';
  const canEdit  = !isRetrieved && !isAdmin && assigneeId !== null;
  const fullName = `${currentProfile.first_name} ${currentProfile.last_name}`.trim();

  document.getElementById('signDocContent').innerHTML = `
    <div class="sign-layout" style="display:flex;height:100%;gap:0">

      <!-- LEFT PANEL — signature tools -->
      <div class="sign-left-panel" style="width:220px;min-width:220px;padding:16px;border-right:1px solid var(--border);overflow-y:auto;display:flex;flex-direction:column;gap:12px">
        <div class="card-title"><i class="ti ti-pen"></i> Signatures</div>
        ...
        <div id="createSigPanel">
          ...
          <div id="typeSigPanel" style="display:none">
            <input type="text" id="typedSigInput" placeholder="Type your name"
              style="width:100%;border:1px solid var(--border-strong);border-radius:6px;padding:8px 10px;font-family:'Dancing Script','Lucida Handwriting',cursive;font-size:18px;color:#000;text-align:center"/>
            <div id="typedSigPreview"
              style="font-family:'Dancing Script','Lucida Handwriting',cursive;font-size:22px;color:#000;text-align:center;padding:8px;border:1px dashed var(--border);border-radius:6px;margin-top:6px;min-height:40px"></div>
            <button class="btn btn-primary btn-sm" style="width:100%;margin-top:6px" onclick="saveTypedSignature()"><i class="ti ti-check"></i> Use</button>
          </div>
        </div>` : ''}

        <div>
          <div style="font-size:11px;font-weight:600;color:var(--text-muted);margin-bottom:6px">PLACED SIGNATURES</div>
          <div id="placedSigList" style="font-size:11px;color:var(--text-muted)">None yet.</div>
        </div>
        ...
        <div style="margin-top:auto;padding-top:12px;border-top:1px solid var(--border)">
          ...
          <div style="font-size:10px;color:var(--text-muted);text-align:center;margin-top:4px">Signatures save live. All assignees can see them.</div>
        </div>` : `<div style="font-size:11px;color:var(--text-muted);text-align:center;padding-top:8px">${isAdmin ? 'Admin view — all signatures shown.' : 'Document is read-only.'}</div>`}
      </div>

      <!-- RIGHT PANEL — full document -->
      <div class="sign-right-panel" style="flex:1;position:relative;overflow:hidden">
        <iframe id="signDocFrame" src="${doc.file_url}#toolbar=0&navpanes=0&scrollbar=0"
          style="width:100%;height:100%;border:none;display:block;position:absolute;top:0;left:0"></iframe>
        <div id="signatureOverlay"
          style="position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;cursor:default"
  ...
  // Restore saved signature if manager/employee
  if (canEdit) {
  ...
  updatePlacedSigList();
  if (canEdit) setupDragResize();

  // Poll for new signatures every 5 seconds (live doc)
  signPollingInterval = setInterval(async () => {
    const { data: fresh } = await db.from('document_signatures').select('*').eq('document_id', docId);
    if (!fresh) return;
    // Merge: keep local unsaved + update from DB
...
function initSignatureCanvas() {
  ...
  signatureCtx.lineWidth   = 2; signatureCtx.lineCap = 'round'; signatureCtx.lineJoin = 'round';
  const getPos = e => {
...
function saveDrawnSignature() {
  ...
  showToast('Signature saved. Click "Place on Document".');
}
...
function showSavedSignature(data, type) {
  ...
  display.innerHTML = type === 'draw'
    ? `<img src="${data}" style="max-width:100%;max-height:50px;object-fit:contain"/>`
    : `<span style="font-family:'Dancing Script','Lucida Handwriting',cursive;font-size:22px;color:#000">${escapeHtml(data)}</span>`;
...
function handleDocClick(event) {
  ...
  const overlay = document.getElementById('signatureOverlay'); if (overlay) overlay.style.cursor = 'default';
  ...
  renderPlacedSignatures(); updatePlacedSigList(); autoSaveSig(sig);
}

async function autoSaveSig(sig) {
  const fullName = `${currentProfile.first_name} ${currentProfile.last_name}`.trim();
  const { data, error } = await db.from('document_signatures').insert({
    document_id: currentSignDocId, signer_id: currentUser.id, signer_name: fullName, signer_email: currentUser.email,
    signature_type: sig.type, signature_data: sig.data, position_x: sig.x, position_y: sig.y,
    size: sig.size || 40, signed_at: new Date().toISOString()
  }).select().single();
...
/* FIXED: Black signatures, no underline, all users' signatures visible */
function renderPlacedSignatures() {
  ...
  placedSignatures.forEach(sig => {
    ...
    div.style.cssText = `position:absolute;left:${sig.x}%;top:${sig.y}%;pointer-events:${canEdit && sig.signerId === currentUser.id ? 'auto' : 'none'};cursor:${canEdit && sig.signerId === currentUser.id ? 'move' : 'default'};user-select:none;`;
    ...
    const content = sig.type === 'draw'
      ...
      : `<span style="font-family:'Dancing Script','Lucida Handwriting',cursive;font-size:${fontSize}px;color:#000;display:block;white-space:nowrap">${escapeHtml(sig.data)}</span>`;

    div.innerHTML = content
      + (isOwn && canEdit ? `<button class="sig-remove-btn" onclick="event.stopPropagation();removeSig('${sig.id}')" style="position:absolute;top:-8px;right:-8px;width:18px;height:18px;border-radius:50%;border:none;background:#dc2626;color:white;font-size:10px;cursor:pointer;display:flex;align-items:center;justify-content:center;line-height:1">×</button>` : '')
      + (isOwn && canEdit ? `<div class="sig-resize-handle" data-sigid="${sig.id}" style="position:absolute;bottom:-4px;right:-4px;width:12px;height:12px;background:var(--brand);border-radius:2px;cursor:se-resize"></div>` : '');
...
function sigDragStart(e) {
  ...
  const el = e.currentTarget; const sigId = el.id.replace('sigEl_',''); activeSigId = sigId;
  const rect = el.getBoundingClientRect(); dragOffset = { x: e.clientX - rect.left, y: e.clientY - rect.top }; e.preventDefault();
  const onMove = ev => {
    ...
    el.style.left = Math.max(0, Math.min(98, newX)) + '%'; el.style.top = Math.max(0, Math.min(98, newY)) + '%';
  };
  const onUp = async ev => {
...
function setupDragResize() {
  document.addEventListener('mousedown', async e => {
    ...
    const sigId = e.target.dataset.sigid; const sig = placedSignatures.find(s => String(s.id) === String(sigId)); if (!sig) return;
    isResizing = true; resizeSigId = sigId; resizeStartSize = sig.size || 40; resizeStartY = e.clientY; e.preventDefault();
    const onMove = async ev => {
      const newSize = Math.max(20, Math.min(120, resizeStartSize + (ev.clientY - resizeStartY)));
      sig.size = newSize;
...
function updatePlacedSigList() {
  ...
  if (placedSignatures.length === 0) { el.innerHTML = '<div style="font-size:11px;color:var(--text-muted);font-style:italic">None yet.</div>'; return; }
  // Group by signer
  const bySigner = {};
  placedSignatures.forEach(sig => {
    if (!bySigner[sig.signerName]) bySigner[sig.signerName] = 0;
    bySigner[sig.signerName]++;
  });
  el.innerHTML = Object.entries(bySigner).map(([name, count]) => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:3px 0;border-bottom:1px solid var(--border)">
      <span style="font-size:11px;color:var(--text-secondary)">${escapeHtml(name)}</span>
      <span style="font-size:10px;color:var(--text-muted)">${count} sig${count !== 1 ? 's' : ''}</span>
    </div>`).join('');
}
...
async function saveUploadedDocument() {
  ...
  showToast('Document uploaded. Now assign it.');
  await renderSignAdminView();
}
...
function closeModal(id) {
  ...
  // Stop polling when sign modal closes
  if (id === 'signDocModal') {
    if (signPollingInterval) { clearInterval(signPollingInterval); signPollingInterval = null; }
  }
}
