...
async function openSignDocModal(docId, assigneeId) {
  ...
  if(signBtn){signBtn.style.display=assignee?.signed?'none':'inline-flex';signBtn.onclick=()=>signDocument(docId,assigneeId);}
  const fullName=`${currentProfile.first_name} ${currentProfile.last_name}`.trim();
  ...
  document.getElementById('signDocContent').innerHTML=`
    ...
    <iframe class="sign-pdf-viewer" src="${doc.file_url}#toolbar=0"></iframe>
    <div class="signature-pad-wrap" style="margin-top:14px">
      ${assignee?.signed
        ?`<div style="text-align:center"><i class="ti ti-circle-check" style="font-size:28px;color:var(--success);display:block;margin-bottom:8px"></i><div style="font-size:13px;font-weight:600">You have signed this document</div><div style="font-size:11px;color:var(--text-muted)">${new Date(assignee.updated_at||assignee.created_at).toLocaleString('en-ZA')}</div></div>`
        :`<p style="font-size:13px;color:var(--text-secondary);margin-bottom:10px">By clicking "Sign Document" you confirm that you have reviewed and agree to this document. Your name and timestamp will be recorded as your electronic signature.</p>
          <div style="font-family:'DM Serif Display',serif;font-size:22px;color:var(--brand-dark);padding:14px 18px;border:2px dashed var(--border-strong);border-radius:8px;text-align:center">${escapeHtml(fullName)}</div>`}
    </div>
    ${allSigs&&allSigs.length>0?`<div class="signature-list" style="margin-top:14px"><div class="info-label" style="margin-bottom:8px">Signatures Recorded</div>${allSigs.map(s=>`<div class="signature-row"><span><i class="ti ti-pen" style="color:var(--success);margin-right:6px"></i>${escapeHtml(s.signer_name)}</span><span style="color:var(--text-muted);font-size:12px">${new Date(s.signed_at).toLocaleDateString('en-ZA',{day:'numeric',month:'short',year:'numeric'})}</span></div>`).join('')}</div>`:''}`;
