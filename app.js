...
async function deleteAccount(profileId, fullName) {
  ...
  showToast(`${fullName} has been deleted.`);
  await renderAccountManagement();
}
...
async function retrieveDocument(docId) {
  ...
  if (sigCount === 0) { if (!confirm('No signatures placed yet. Download anyway?')) return; }
  else { alert(`This document has ${sigCount} placed signature(s) saved for audit. The original PDF will now download.`); }
