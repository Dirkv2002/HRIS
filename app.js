...
let signPollInterval         = null; // free polling instead of paid realtime
...
/* ===========================================
   EMAIL — fire-and-forget (no awaiting = fast UI)
=========================================== */
const FROM_EMAIL = 'dvermeulen@communicare.org.za';

function sendEmail(toEmail, subject, body) {
  // Fire and forget — don't await so UI is never blocked by email sending
...
/* FIXED: fire-and-forget emails + parallel DB writes = near-instant */
async function informManagerOfShortlist() {
  ...
  await Promise.all(shortlisted.map(async app => {
    ...
    if (existing) {
      return db.from('shortlists').update({ list_type: 'shortlist', updated_at: new Date().toISOString() }).eq('id', existing.id);
    }
    return db.from('shortlists').insert({ vacancy_id: String(currentVacancyId), vacancy_title: vac?.title || '', application_id: app.id, candidate_name: app.candidate_name, candidate_email: app.candidate_email, list_type: 'shortlist', score: app.score, created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
  }));

  // Fire-and-forget emails — do NOT await
  db.from('profiles').select('email,first_name,last_name').eq('role', 'manager').eq('department', vac?.department || '').then(({ data: deptManagers }) => {
    (deptManagers || []).forEach(mgr => sendManagerShortlistEmail(mgr.email, `${mgr.first_name} ${mgr.last_name}`.trim(), vac?.title || 'the vacancy'));
  });
...
async function finaliseApplicationStatuses() {
  ...
  await Promise.all(apps.map(async app => {
    ...
    if (listType === 'rejected') {
      ...
      await Promise.all([
        db.from('applications').update({ status: 'rejected' }).eq('id', app.id),
        db.from('profiles').update({ status: 'rejected' }).eq('email', app.candidate_email),
        db.from('shortlists').delete().eq('application_id', app.id)
      ]);
      sendRejectionEmail(app.candidate_email, app.candidate_name, app.vacancy_title);
    } else if (listType === 'longlist') {
      await Promise.all([
        db.from('applications').update({ status: 'reviewing' }).eq('id', app.id),
        db.from('profiles').update({ status: 'reviewing' }).eq('email', app.candidate_email),
        db.from('shortlists').delete().eq('application_id', app.id)
      ]);
    } else if (listType === 'shortlist') {
      await Promise.all([
        db.from('applications').update({ status: 'interview1' }).eq('id', app.id),
        db.from('profiles').update({ status: 'interview1' }).eq('email', app.candidate_email)
      ]);
...
/* ===========================================
   AVAILABILITY — only invited users see calendar
=========================================== */
async function renderAvailabilityVacancies() {
  ...
  if (list.length === 0) {
    html += `<div class="card" style="text-align:center;padding:40px">
      ...
      <div style="font-size:14px;color:var(--text-muted)">${userRole === 'admin' ? 'No vacancies yet.' : 'You have not been invited to any availability calendars. An administrator will invite you when needed.'}</div>
    </div>`;
  } else {
    html += list.map(v => `
...
async function renderOnboardingSlots() {
  ...
  if (slots.length === 0) {
    html += `<div class="card" style="text-align:center;padding:40px">
      ...
      <div style="font-size:14px;color:var(--text-muted)">${userRole === 'admin' ? 'No onboarding slots yet. Click "Add Slot".' : 'No onboarding slots available yet.'}</div>
    </div>`;
  } else {
...
async function addOnboardingSlot() {
  ...
  if (!date || !time) { alert('Please select date and time.'); return; }
  // FIXED: no vacancy_id — column does not exist in this table
...
/* ===========================================
   ...
   ARCHITECTURE:
   - The document sits in a scrollable dark panel (like Adobe)
   - Signature overlay is INSIDE the sign-doc-frame-wrap (same scroll container)
   - All coordinates are relative to sign-doc-frame-wrap
   - Polling every 3 seconds fetches all signatures (free, no Realtime needed)
   - Signatures are always black
=========================================== */
async function openSigningModal(docId, assigneeId, alreadySigned, isRetrieved) {
  ...
  // Load ALL signatures for this document (all users — live doc)
  ...
  document.getElementById('signDocContent').innerHTML = `
    <div class="sign-layout">
      ...
      <div class="sign-left-panel">
        ...
        <div style="font-size:10px;color:var(--text-muted);background:var(--surface-2);padding:8px;border-radius:6px;line-height:1.5;margin-top:auto">
          <i class="ti ti-refresh"></i> Refreshes every 3 seconds. All assignees see the same signatures.
        </div>
        ...
        <div style="font-size:11px;color:var(--text-muted);text-align:center">
          ${isAdmin ? 'Admin — viewing all signatures.' : 'Read only.'}
        </div>`}
      </div>

      <!-- RIGHT PANEL: scrollable dark background, document fills it -->
      <div class="sign-right-panel">
        <!--
          IMPORTANT: The overlay is INSIDE sign-doc-frame-wrap so coordinates
          are always relative to the same element, fixing the drag-away bug.
        -->
        <div class="sign-doc-frame-wrap" id="signFrameWrap">
          <iframe id="signDocFrame"
            src="${doc.file_url}#toolbar=0&navpanes=0&scrollbar=1"
            style="width:100%;height:100%;min-height:1100px;border:none;display:block;position:relative;z-index:1">
          </iframe>
          <!-- Overlay sits on top of iframe, same coordinate space -->
  ...
  // Restore saved signature for editor
  if (canEdit) {
  ...
  // Start 3-second poll — free alternative to Realtime
  startSignPoll(docId);
}

/* Free polling — replaces paid Realtime subscription */
...
async function reloadAllSignatures(docId) {
  ...
  // Preserve any unsaved local placements, update rest from DB
  ...
  // Merge: DB sigs + any locally placed but not yet saved
...
function initSignatureCanvas() {
  ...
  signatureCtx.lineWidth   = 2.5; signatureCtx.lineCap = 'round'; signatureCtx.lineJoin = 'round';
  const getPos = e => {
...
/* FIXED: use signFrameWrap bounding rect consistently for click placement */
function handleDocClick(event) {
  ...
  // Use the wrap element — same reference used during drag
...
async function removeSig(sigId) {
  ...
  if (sig.saved && !String(sigId).startsWith('temp_')) {
...
/* FIXED: drag uses signFrameWrap rect consistently — no more "running away" bug */
...
/* FIXED: drag calculates offset from signFrameWrap — same reference as placement */
function sigDragStart(e) {
  ...
  // Get wrap rect once at drag start
  const wrap      = document.getElementById('signFrameWrap');
  if (!wrap) return;
  const wrapRect  = wrap.getBoundingClientRect();

  // Calculate offset within the signature element itself
  const sigRect   = el.getBoundingClientRect();
  dragOffset      = { x: e.clientX - sigRect.left, y: e.clientY - sigRect.top };

  const onMove = ev => {
    // All positions relative to wrap
    const newX = ((ev.clientX - wrapRect.left - dragOffset.x) / wrapRect.width)  * 100;
    const newY = ((ev.clientY - wrapRect.top  - dragOffset.y) / wrapRect.height) * 100;
    el.style.left = Math.max(0, Math.min(97, newX)) + '%';
    el.style.top  = Math.max(0, Math.min(97, newY)) + '%';
  };
  ...
  const onUp = async ev => {
    ...
    if (sig) {
      const newX = ((ev.clientX - wrapRect.left - dragOffset.x) / wrapRect.width)  * 100;
      const newY = ((ev.clientY - wrapRect.top  - dragOffset.y) / wrapRect.height) * 100;
      sig.x = parseFloat(Math.max(0, Math.min(97, newX)).toFixed(2));
      sig.y = parseFloat(Math.max(0, Math.min(97, newY)).toFixed(2));
...
function setupDragResize() {
  // Use a named handler so we can track it (only set up once per modal open)
  const handler = async e => {
    if (!e.target.classList.contains('sig-resize-handle')) return;
    const sigId = e.target.dataset.sigid;
    const sig   = placedSignatures.find(s => String(s.id) === String(sigId));
    if (!sig) return;
    isResizing = true; resizeSigId = sigId; resizeStartSize = sig.size || 40; resizeStartY = e.clientY;
...
/* FIXED: parallel inserts + fire-and-forget emails = instant response */
async function confirmAssignDocument() {
  ...
  // Delete old and re-insert in parallel with email fire-and-forget
  ...
  await Promise.all([...checkboxes].map(cb =>
    db.from('document_assignees').insert({
      document_id: pendingAssignDocId, manager_id: cb.value,
      manager_name: cb.dataset.name, manager_email: cb.dataset.email,
      signed: false, created_at: new Date().toISOString()
    })
  ));

  // Fire emails after UI is already done — does not block
