...
/* ===========================================
   EMAIL HELPERS — parallel sending for speed
=========================================== */
...
async function sendInterviewConfirmationEmail(toEmail, n, d, t){ await sendEmail(toEmail, `Interview Confirmed — Communicare`, `Dear ${n},\n\nYour interview has been booked for ${d} at ${t}.\n\nKind regards,\nCommunicare Human Capital Team`); }
...
async function sendManagerShortlistEmail(email, n, t)          { await sendEmail(email, `Shortlist ready — ${t}`, `Dear ${n},\n\nA shortlist for "${t}" is ready for your review.\n\nKind regards,\nCommunicare Human Capital Team`); }
async function sendManagerDocumentEmail(email, n, t)           { await sendEmail(email, `Document to sign — ${t}`, `Dear ${n},\n\nDocument "${t}" has been assigned for your signature. Log in and go to the Sign tab.\n\nKind regards,\nCommunicare Human Capital Team`); }
async function sendCandidateInterviewInviteEmail(email, n, t)  { await sendEmail(email, `Interview invitation — ${t}`, `Dear ${n},\n\nYou have been shortlisted for "${t}". Please log in and book your interview slot in the Bookings tab.\n\nKind regards,\nCommunicare Human Capital Team`); }
...
/* FIXED: parallel email sending for speed */
async function informManagerOfShortlist() {
  ...
  // Save shortlist entries in parallel
  await Promise.all(shortlisted.map(async app => {
  ...
  // Send emails to dept managers in parallel
  const { data: deptManagers } = await db.from('profiles').select('email,first_name,last_name').eq('role', 'manager').eq('department', vac?.department || '');
  await Promise.all((deptManagers || []).map(mgr => sendManagerShortlistEmail(mgr.email, `${mgr.first_name} ${mgr.last_name}`.trim(), vac?.title || 'the vacancy')));

  showToast('Shortlist sent to manager.');
...
async function renderAvailabilityVacancies() {
  ...
  if (userRole !== 'admin') {
    ...
    (inviteRows || []).forEach(row => {
      ...
      if (ids.includes(currentUser.id)) myInvitedIds.add(String(row.vacancy_id));
    });
    list = list.filter(v => myInvitedIds.has(String(v.id)));
  }

  let html = `
    <div class="page-header" style="padding-top:16px">
      <h1 style="font-size:20px">Team Availability</h1>
      <p>${userRole === 'admin' ? 'Manage team availability per vacancy and invite members.' : 'Mark your availability for interview panels you have been invited to.'}</p>
    </div>
  ...
  if (list.length === 0) {
    html += `<div class="card" style="text-align:center;padding:40px">
      ...
      <div style="font-size:14px;color:var(--text-muted)">${userRole === 'admin' ? 'No vacancies yet.' : 'You have not been invited to any interview availability calendars yet. An administrator will invite you when needed.'}</div>
    </div>`;
  } else {
    html += list.map(v => `
...
/* ===========================================
   ONBOARDING SLOTS — FIXED: removed vacancy_id from insert
=========================================== */
async function renderOnboardingSlots() {
  const panel = document.getElementById('bookingPanel-onboarding'); if (!panel) return;

  // For onboarding, no vacancy filter since vacancy_id may not exist in table
  ...
  if (slots.length === 0) {
    html += `<div class="card" style="text-align:center;padding:40px">
      ...
      <div style="font-size:14px;color:var(--text-muted)">${userRole === 'admin' ? 'No onboarding slots yet. Click "Add Slot" to create one.' : 'No onboarding slots available yet.'}</div>
    </div>`;
  } else {
...
async function addOnboardingSlot() {
  ...
  if (!date || !time) { alert('Please select date and time.'); return; }
  // FIXED: removed vacancy_id which doesn't exist in onboarding_slots table
...
async function cancelOnboardingSlot(slotId) {
  if (!confirm('Cancel this onboarding session?')) return;
...
async function retrieveDocument(docId) {
  ...
  const sigCount    = (sigsRes.data || []).length;
  if (!confirm(`${doc.title}\n\n${signedCount}/${totalCount} signed · ${sigCount} placements\n\nDownload?`)) return;
...
/* ===========================================
   SIGNING MODAL
   - Adobe + Google Docs hybrid: live, scrollable
   - Real-time via Supabase Realtime subscriptions
   - All signatures from all users always visible
...
async function openSigningModal(docId, assigneeId, alreadySigned, isRetrieved) {
  ...
  // Unsubscribe from any previous realtime channel
  if (signRealtimeChannel) {
    await db.removeChannel(signRealtimeChannel);
    signRealtimeChannel = null;
  }

  const { data: doc } = await db.from('sign_documents').select('*').eq('id', docId).single();
  if (!doc) return;

  // Load ALL signatures for this document (all users)
  ...
  document.getElementById('signDocContent').innerHTML = `
    <div class="sign-layout">
      ...
      <div class="sign-left-panel">
        <div class="card-title" style="font-size:13px"><i class="ti ti-pen"></i> Signatures</div>
        ...
        <div id="savedSigPreview" style="display:none;border:2px solid var(--border);border-radius:8px;padding:8px;text-align:center;background:var(--surface-2)">
          ...
          <button class="btn btn-primary btn-sm"   style="margin-top:4px;width:100%" onclick="activatePlacement()"><i class="ti ti-cursor-text"></i> Place on Doc</button>
        </div>
        ...
        <div>
          <div style="font-size:11px;font-weight:600;color:var(--text-muted);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px">Live Signatures</div>
          <div id="placedSigList" style="font-size:11px;color:var(--text-muted)">None yet.</div>
        </div>

        <div style="font-size:10px;color:var(--text-muted);padding:8px;background:var(--surface-2);border-radius:6px;line-height:1.5">
          <i class="ti ti-wifi"></i> Live — signatures appear instantly for all viewers.
        </div>
        ...
        </div>` : `<div style="font-size:11px;color:var(--text-muted);text-align:center">${isAdmin ? 'Admin view — all signatures shown.' : 'Read only.'}</div>`}
      </div>

      <!-- RIGHT: Scrollable document -->
      <div class="sign-right-panel">
        <div class="sign-doc-frame-wrap" id="signFrameWrap">
          <iframe id="signDocFrame" src="${doc.file_url}#toolbar=0&navpanes=0"
            style="width:100%;height:100%;min-height:1100px;border:none;display:block"></iframe>
  ...
  /* =====================================================
     SUPABASE REALTIME — instant cross-user signature sync
     Subscribes to INSERT/UPDATE/DELETE on document_signatures
     so all open clients see changes immediately
  ===================================================== */
  signRealtimeChannel = db.channel(`doc-sigs-${docId}`)
    .on('postgres_changes', {
      event: '*',
      schema: 'public',
      ...
      if (payload.eventType === 'INSERT') {
        ...
        if (!placedSignatures.find(p => p.id === s.id)) {
        ...
        const s = payload.new;
        const existing = placedSignatures.find(p => p.id === s.id);
        if (existing) {
          existing.x = s.position_x; existing.y = s.position_y; existing.size = s.size || existing.size;
          renderPlacedSignatures(); updatePlacedSigList();
        }
      } else if (payload.eventType === 'DELETE') {
        placedSignatures = placedSignatures.filter(p => p.id !== payload.old.id);
        renderPlacedSignatures(); updatePlacedSigList();
      }
    })
    .subscribe();
...
function initSignatureCanvas() {
  ...
  signatureCtx.strokeStyle = '#000'; signatureCtx.lineWidth = 2; signatureCtx.lineCap = 'round'; signatureCtx.lineJoin = 'round';
  const getPos = e => {
...
function saveDrawnSignature() {
  ...
  showToast('Signature ready. Click "Place on Doc".');
}
...
async function removeSig(sigId) {
  if (currentSignDocRetrieved) { alert('Document retrieved — cannot remove signatures.'); return; }
...
/* FIXED: Black signatures, no underline, relative to scrollable container */
function renderPlacedSignatures() {
  ...
  placedSignatures.forEach(sig => {
    ...
    div.style.cssText = `left:${sig.x}%;top:${sig.y}%;pointer-events:${canEdit && sig.signerId === currentUser.id ? 'auto' : 'none'};`;

    const fontSize = (sig.size || 40) * 0.6;
    const isOwn    = sig.signerId === currentUser.id;

    // Black drawn signature via filter, black typed signature via color
...
function sigDragStart(e) {
  ...
  const onMove = ev => {
    ...
    el.style.left = Math.max(0, Math.min(98, newX)) + '%';
    el.style.top  = Math.max(0, Math.min(98, newY)) + '%';
  };
...
async function submitDocumentSignature() {
  if (currentSignDocRetrieved) { alert('Document has been retrieved.'); return; }
...
/* FIXED: parallel assignment + email sending for speed */
async function confirmAssignDocument() {
  ...
  // Insert all + send emails in parallel
  await Promise.all([...checkboxes].map(async cb => {
...
function closeModal(id) {
  ...
  // Clean up realtime channel when sign modal closes
  if (id === 'signDocModal' && signRealtimeChannel) {
    db.removeChannel(signRealtimeChannel);
    signRealtimeChannel = null;
  }
}
