...
let userRole          = 'candidate'; // 'admin' | 'manager' | 'candidate'
let slotsData         = [];
let vacanciesData     = [];
...
document.addEventListener('DOMContentLoaded', () => {
  // Show dept selector when manager role selected on signup
...
async function uploadCV(e) {
  ...
  if (!error) {
    ...
    if (data?.publicUrl) await db.from('profiles').update({ cv_url: data.publicUrl }).eq('id', currentUser.id);
  }
          ...
          return `
            <div class="applicant-row">
              ...
              <div class="applicant-row-actions">
                <span class="status-badge ${cfg.cls}" style="margin:0;font-size:10px">
                  ${cfg.label}
                </span>
                ...
                <button class="btn btn-primary" style="font-size:11px;padding:4px 10px"
                  onclick="promptStatusChange('${app.id}','${app.candidate_name}','${app.status}')">
  ...
  content.innerHTML = `
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <button class="btn btn-primary"
        onclick="closeModal('candidateProfileModal');promptStatusChange('${app.id}','${app.candidate_name}','${app.status}')">
...
async function updateApplicationStatus(appId, newStatus, candidateName) {
  ...
  // Also update profile status if candidate
  const { data: app } = await db.from('applications').select('*').eq('id', appId).single();
  if (app) {
...
/* ═══════════════════════════════════════════
   EMAIL NOTIFICATIONS (via Supabase table)
   Note: Wire up a Supabase Edge Function or
   external service (e.g. Resend, SendGrid)
   to watch this table and send emails.
═══════════════════════════════════════════ */
...
We wanted to let you know that your application for the position of ${vacancyTitle} at Communicare has been updated.

Your current application status is: ${newStatus}

We appreciate your interest in joining the Communicare team and will be in touch with further details soon.

Kind regards,
Communicare Human Capital Team`;

  await db.from('email_notifications').insert({ to_email: toEmail, subject, body });
}
...
Thank you sincerely for taking the time to apply for the position of ${vacancyTitle} at Communicare and for your interest in joining our team.

After careful consideration of all applications received, we regret to inform you that we will not be moving forward with your application at this time. This was a difficult decision, as we received many strong applications.

We truly appreciate the time and effort you invested in the application process. We encourage you to continue pursuing opportunities that align with your skills and aspirations, and we wish you every success in your career journey.

Please do not hesitate to apply for future positions at Communicare that match your profile.
...
Format: As communicated by our team

Please ensure you are available and prepared at the scheduled time. Should you need to cancel or reschedule, please do so via the recruitment portal as soon as possible.
...
If you would like to rebook, please visit the recruitment portal and select a new available slot.
          ...
          <div class="strategy-item">
            ...
            <div class="grid-2" style="margin-bottom:12px">
              <div>
                ...
                <div style="font-size:13px;color:var(--text-primary)">${escapeHtml(s.qualifications || '—')}</div>
              </div>
              <div>
                <div class="report-label">Experience</div>
                <div style="font-size:13px;color:var(--text-primary)">${escapeHtml(s.experience || '—')}</div>
              </div>
              <div>
...
function renderDepts() {
  ...
  // Show Add Job button for admins only
  ...
  for (const [dept, jobs] of Object.entries(depts)) {
    ...
    block.innerHTML = `
      <div class="dept-header" onclick="toggleDept(this)">
        <h4><i class="ti ti-building"></i>${dept}<span class="dept-count">${(jobs||[]).length} roles</span></h4>
        <i class="ti ti-chevron-down chevron"></i>
      </div>
...
const assessmentQuestions = [
  { q:'Which of the following best describes RESTful API design principles?', opts:['Stateful client-server communication','Stateless communication using standard HTTP methods','Requires WebSocket connections','Only works with JSON'], correct:1 },
  { q:'What does SOLID stand for in software engineering?', opts:['A set of object-oriented design principles','A database management framework','A programming language specification','A testing methodology'], correct:0 },
  { q:'In agile, what is the primary purpose of a sprint retrospective?', opts:['Plan the next sprint','Review and improve team processes','Demo features to stakeholders','Assign tasks to developers'], correct:1 },
  { q:'Which version control practice best supports code review?', opts:['Committing directly to main','Using feature branches with pull requests','Deleting branches after each commit','Working in a single shared branch'], correct:1 },
  { q:'What is the main advantage of containerisation (e.g. Docker)?', opts:['Eliminates the need for testing','Consistent environments across development and production','Automatically writes code','Replaces version control'], correct:1 }
];
  ...
  el.innerHTML = questions.map((q, qi) => `
    <div class="sq-item">
      ...
      <div style="display:flex;gap:8px;margin-top:8px">
        <button class="btn btn-secondary" style="font-size:11px;padding:4px 10px" onclick="addSQOption(${qi})">
          <i class="ti ti-plus"></i> Add Option
        </button>
        <button class="btn btn-danger" style="font-size:11px;padding:4px 10px" onclick="removeSQQuestion(${qi})">
...
function openApplyModal(vacId) {
  ...
  el.innerHTML = questions.length === 0
    ? `<div class="notice"><i class="ti ti-info-circle"></i>
        <span>No screening questions for this role. Just submit your application.</span></div>`
    : questions.map((q, qi) => `
        <div class="q-card">
...
async function submitApplication() {
  ...
  // Update progress
  await db.from('profiles').update({ status: 'applied' }).eq('id', currentUser.id);
  currentProfile.status = 'applied';
  applyStatus('applied');
  ...
  // Show confirmation
  showToast(`Application submitted! Fit: ${score >= 70 ? '🟢 Good' : score >= 40 ? '🟡 Moderate' : '🔴 Under review'}`);
...
async function openApplicantListModal(vacId) {
  ...
  const { data, error } = await db.from('applications').select('*').eq('vacancy_id', vacId).order('score', { ascending:false });
...
function bulkAdvance() {
  const checked = document.querySelectorAll('.bulk-check:checked');
  if (checked.length === 0) { alert('Select candidates first.'); return; }
  bulkAdvanceVacancy(currentVacancyId);
}

function bulkReject() {
  bulkRejectVacancy(currentVacancyId);
}
...
function renderBookingTabs() {
  ...
  bar.innerHTML = myTabs.map((t,i) => `
    <button class="booking-tab-btn ${i===0?'active':''}"
      id="bookingTabBtn_${t}"
      onclick="switchBookingTab('${t}')">
      <i class="ti ${labels[t].icon}"></i> ${labels[t].label}
    </button>`).join('');

  // Show/hide add slot buttons
...
async function renderSlots() {
  ...
  slotsData.forEach(slot => {
    ...
    // Candidate can only book if they have no existing booking
    ...
    } else if (!slot.booked_by_email && userRole === 'admin') {
      actionHtml = `<div style="margin-top:10px">
        <button class="btn btn-secondary" style="width:100%;font-size:11px;padding:5px;justify-content:center" disabled>
          Available
        </button></div>`;
    }
  ...
  if (myBooking && userRole === 'candidate') {
    ...
    notice.innerHTML = `<i class="ti ti-info-circle"></i>
      <span>You have already booked an interview slot. Cancel your existing booking to book a different slot.</span>`;
...
═══════════════════════════════════════════ */
let myAvailability = new Set(); // "date|time" strings
let allAvailability = [];       // from DB
...
async function renderAvailabilityCalendar() {
  ...
  // Time slots 09:00–15:00 in 30min increments
  const times = [];
  for (let h = 9; h < 15; h++) {
  ...
  // Build table
  const uniqueUsers = [...new Set(allAvailability.map(r => r.user_id))];
  const totalUsers  = Math.max(uniqueUsers.length, 1);

  let html = `<div class="availability-calendar-wrap"><table class="availability-table">
    <thead><tr>
  ...
  times.forEach(time => {
    ...
    days.forEach(day => {
      ...
      const isMine    = myAvailability.has(key) || myAvailability.has(keyS);
...
function renderRecommendedSlots(days, times, totalUsers) {
  ...
  if (recommended.length === 0) {
    el.innerHTML = '<p style="font-size:13px;color:var(--text-muted)">No slots where all members are available yet.</p>';
  } else {
...
async function renderOnboardingSlots() {
  ...
  if (slots.length === 0) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:30px;color:var(--text-muted)">No onboarding slots available yet.</div>';
    return;
  }
...
function confirmCancelOnboarding(slotId, name, date, time) {
  // Only admin can cancel others; managers can only cancel their own
  if (userRole !== 'admin') {
    const slot = document.querySelector(`[onclick*="${slotId}"]`);
    // Extra check done server-side via RLS
  }
